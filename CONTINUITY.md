...
### Now
- **2026-09-02 Coach's brief B1 fixture bridge delivered in PR #975 (open, unmerged), review round complete:** bridged local players now mint/repair the negative synthetic identity required for result recording after the shared legacy-conflict check; hosted-production targets are refused before connection; non-SQLite database names are belted with an explicit override; repair semantics are documented. Requested bridge/club-console tests 60 passed; backend-wide Ruff check/format clean. No basecamp/prod execution. See `ledgers/CONTINUITY_dev-club-fixture-bridge.md`.
