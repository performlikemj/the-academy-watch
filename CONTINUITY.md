...
### Done
- **2026-09-24 iOS GOL fix round 1:** root-owned conversation survives dismiss/reopen; Ask GOL on Scout Desk/Home; neutral combined quota; inline Markdown without link actions; 401/402/403 orphan cleanup and empty-history filtering; 300s stream timeout. Debug/Release builds and 227 unit/API tests pass; offline UI 16 tests (1 optional local integration skip) includes all 7 journeys, with final GOL rerun 4 tests (same skip). Screenshots/logs: `~/codex-runs/gol-fix1-{shots,logs}/`. No `ledgers/` edits.
...
### Now
- **2026-09-08 iOS 1.1.0 APPROVED → owner found the scout Home bug → 1.1.1 SUBMITTED same day (WAITING_FOR_REVIEW, version `4eb67703…`, Xcode Cloud run #10 → build 10):** a user who chose Scout was parked on the Home tab with player cards and a dead "Change my home" menu. Fix #1065 (`RootTab.available(for:)` / `RootTab.initial(...)`; scouts get the 4-tab layout + Scout Desk on launch; role control in Account → Home experience; `ExperienceRole.storageKey`). Fable checker PASS (203 unit + 1 offline UI test); dev build installed on MJ's phone. Follow-up: harness `DIRECTIVE_story_map.md` (user-story inventory → proof journeys → generated map) so a machine catches this class before a release; A2 adopts it in `academy-watch-ios/sim/` with RED = `292d5912`, GREEN = `5bcdf0c2`.
- **2026-09-06 Dream scorecard re-score: 65.1% → 67.0%.** Billing live + coach→player pilot loop shipped; iOS 1.1.0 submitted, not public. Owner accepted billing WITHOUT a real-card test purchase; revenue $0; App Privacy updated by the owner. Evidence, retained stage history and exact before/after `score.py` outputs: `ledgers/tooling/dream-scorecard/`; ledger: `ledgers/GRADING_dream-scorecard-2026-09-02.md`. Validation: scoring and report/HTML generators rerun; only requested capability scores changed.
- **2026-09-06 iOS 1.1.0 SUBMITTED to App Review (WAITING_FOR_REVIEW, auto-release).** Build 9 on ASC version a9a2109d…; reviewer demo data created in prod without any OTP: player/club/invitation/feedback steps via the public API with the static review accounts (`ledgers/tooling/dream-scorecard/pilot/prod_review_demo.sh`), and the three admin approvals run INSIDE the prod container (`az containerapp exec` through `ptyexec.py`, script sent base64 via stdin; `approve_review_demo_in_container.py` mints a 10-min admin token with `issue_user_token(role="admin")` and calls the admin routes through `app.test_client()`; exec channel rate-limits with 429 — back off ~75 s). Prod now holds: local player 1 "Review Player" (approved, claim 2), club program 1 "Academy Watch Review FC" (manager = review scout account), accepted invitation, feedback 2713d033… with a development action. Owner still to do: App Privacy answers in the ASC web UI (no API), real-card billing acceptance.
- **2026-09-06 iOS 1.1.0 STAGED for App Review (owner: "ready to merge and submit"):** #1045 player onboarding + club experience and #1047 coach development actions (another session's work) merged after Fable checkers (CLEAN / APPROVE_WITH_NOTES, one fix round: raw AI caption no longer prefills coach feedback), #1048 version 1.1.0; migration s4d1 (2 JSON columns) pre-applied + stamped; `PILOT_CLUB_RELATIONSHIPS_ENABLED=1` now ON in prod (P3 lifecycle checks passed). Xcode Cloud run #9 → build 9 (1.1.0) attached to App Store version 1.1.0 with the pack's What's New + promo; review notes extended with the 1.1.0 test paths. NOT yet submitted: reviewer demo data (`ledgers/tooling/dream-scorecard/pilot/prod_review_demo.sh`, needs an ordinary admin OTP) and App Privacy answers (owner, ASC web UI only). Submission pack: `scratchpad/coach-player-development/submission/SUBMISSION_PACK.md`.
- **2026-09-06 coach-to-player development implementation:** AI evidence → coach-authored action → player reflection/progress → coach review on web/iOS. See `ledgers/CONTINUITY_coach-player-development.md`.
- **2026-09-06 Basecamp player/coach QA complete:** 197 iOS unit/API, 8 native dark/XXXL walks (plus 5 original light walks), 22 browser scenarios, and 334 backend tests including PostgreSQL pass. Fixed dark-mode filled-button contrast and stale QA fixtures; screenshots/report available, PR #1045 follow-up. Live deployed-device/email/Azure upload checks remain. See `ledgers/CONTINUITY_ios-basecamp-qa.md`.
- **2026-09-06 iOS player/club experience implemented:** role-oriented Home, persistent My Profiles, unified identity search, adult-age validation, moderated basic-profile/photo/highlight editing, public-profile sharing, native club invitations and private feedback acknowledgment, and club-console handoff. 197 iOS unit/API tests, 5 offline UI journeys, 61 showcase + 24 trust tests, and unsigned archive pass. Local commit `3723bff8` on `feat/ios-player-club-experience`; published with user authorization as draft PR #1045; GitHub CI running. Deploy additive PATCH before binary release and update photo disclosure. See `ledgers/CONTINUITY_ios-player-club-experience.md`.
- **2026-09-06 One-club pilot BUILD COMPLETE (dark): P4 #1043 stable club results (table `club_results`, `player_match_entries.club_result_id`, s4c1) joins P1 #1037, P2 #1036 (s4a1), P3 #1039 (s4b1).** Fable checker 3 rounds on P4 (batched read path had a less-conservative minor gate — fixed). INCIDENT NOTE: the s4c1 pre-apply SQL failed on a formatting slip (two CREATE INDEX statements concatenated) and rolled back, but merge/deploy/stamp proceeded; prod lacked `club_results` + the column for ~5 min until the corrected SQL was applied (0 match-entry rows in prod; no errors observed). Rule: never chain merge after a pre-apply without asserting the "after" table count. Basecamp web sim on main: 11/11 journeys OK. Next: owner outreach (Forest), then the P0 runbook with the real club; `PILOT_CLUB_RELATIONSHIPS_ENABLED` stays off until P3 lifecycle checks pass in prod.
- **2026-09-06 Pilot P3 LIVE (dark): #1039 private player feedback + acknowledgment (new blueprint feedback.py, table player_feedback, migration s4b1 pre-applied + stamped; export/erasure hooks for P2 invitations + feedback).** Astra built; Fable checker 2 rounds (FAIL → PASS: roster panel pagination bug, purge ignoring the flag, HTML-escaped text, irreversible transient closure — all fixed). P4 (result corrections, s4c1) building next.
- **2026-09-06 BILLING LIVE (owner: "enable billing").** Live Stripe: product "Academy Watch chat credits", one-time USD prices Starter $20→100 credits (`gol_starter`) and Top up $10→50 credits (`gol_topup`); new webhook endpoint `api.theacademywatch.com/api/billing/stripe/webhook` (9 events) with its signing secret stored as a new Key Vault version and the container secret repointed; the old endpoint on the dead lemonmoss host disabled. Env: `STRIPE_PRICE_GOL_STARTER`, `GOL_STARTER_CREDITS=100`, `STRIPE_PRICE_GOL_TOPUP`, `GOL_TOPUP_CREDITS=50`, repo var `VITE_BILLING_TERMS=1` (terms effective 2026-09-06, #1040), then `BILLING_ENABLED=1`. Verified: `/api/billing/config` 200 with both packs and no subscription products; health 200. Stripe account is JP/JPY — USD prices settle with FX. **Remaining owner acceptance:** buy the Starter pack once with a real card, confirm 100 credits, refund in the Stripe dashboard, confirm the reversal; watch `product_events` for `gol_orphaned_purchase`. Live ids recorded in the S3 ledger.
- **2026-09-05 One-club pilot wave 1 LIVE (dark): #1037 P1 admin cohort register/report; #1036 P2 claimant-accepted club relationships + local contact routing (migration s4a1 pre-applied + stamped; `PILOT_CLUB_RELATIONSHIPS_ENABLED` default off).** Built by codex gpt-6-astra against its own contracts (`ledgers/tooling/dream-scorecard/pilot/contracts.md`); Fable checkers 2 rounds each (CLEAN/PASS). Wave 2 (P3 feedback, then P4 result corrections) in progress. Directive `ledgers/DIRECTIVE_pilot-club.md`. Note: the two merge deploys collided (ContainerAppOperationInProgress) — rerun fixed it; merge pilot PRs one at a time.
- **2026-09-05 Dream scorecard refreshed to the independent audit — 65.6% → 65.1%** (gpt-6-astra `ledgers/research/astra-review-2026-09-05.md` Part 1, evidence-adjusted): 3.6 3→2 and 5.5 3→2 (billing dark; rail complete incl. the money-safety fixes; →3 at go-live per review B2), 2.7 1→2 (coaching briefs exist); stale evidence refreshed on 1.2, 1.7, 3.2, 3.5, 5.4 and 4.4 reach MISSING→LIVE_WEB. MS stage row + B2 go-live row in `ledgers/GRADING_dream-scorecard-2026-09-02.md`; artifact page regenerated.
- **2026-09-05 Money-safety stage SHIPPED DARK (#1028 club authority, #1029 web SSE+terms, #1030 GOL executions/settlements/terms, migration s3e1 pre-applied + stamped in prod):** fixes the 3 P1 defects + 3 launch blockers from the gpt-6-astra review; policies: answer counts as begun on any substantive event (disconnect after that = credit used; before = refund), withheld disconnects replay as terminal partial answers, refunds before grants are held in `gol_payment_settlements`, purchase terms frozen per Stripe session in `gol_checkout_terms`, orphaned late payments recorded as `gol_orphaned_purchase` for manual refund, club-confirmed results need club authority at entry time. Checkers: 3 Fable rounds each, all CLEAN/PASS. Codex account hit its usage limit at the end (resets 2026-09-09); GLM on basecamp is the fallback executor. Prod: alembic **s3e1**, billing still 404, chat anon 401. **Next = owner go-live checklist** in `ledgers/CONTINUITY_dream-s3.md` (Stripe endpoint, GOL price/credit envs, confirm terms effective date 2026-09-15 → `VITE_BILLING_TERMS=1`, then `BILLING_ENABLED=1`, one real-card purchase + refund).
- **2026-09-04 iOS 1.0.1 SUBMITTED to App Review** (build 8 from Xcode Cloud run #8 on main b48f619; App Store version id c98701b4…, releaseType AFTER_APPROVAL, review notes + demo account carried over from 1.0). Contents: live host (#1010), season-stats decode (#1012), sign-in code cap + UI walks (#1014), version wiring `CFBundleShortVersionString = $(MARKETING_VERSION)` (#1016). Next bump = one line in `project.yml`. Watch: `asc.sh GET /v1/apps/6799866798/appStoreVersions?fields[appStoreVersions]=versionString,appStoreState`.
- **2026-09-04 iOS live UI walks (#1014 LIVE):** XCUITest target `AcademyWatchUITests` drives the app against prod (anonymous walk + reviewer-scout walk with watchlist/list mutations reverted); run via `-scheme AcademyWatchUISmoke` per `academy-watch-ios/AcademyWatchUITests/README.md`. The walk found that App Review could not sign in to build 7: the sign-in field capped codes at 11 chars while the review code is 20 (fixed; unit suite 180). Three Fable checker rounds; last one CLEAN. Remaining product notes: watchlist rows show "Position TBD" when the search summary lacks a position (backend data gap), and the "Sold" pill renders the backend status faithfully. App Store 1.0.1 is now gated only on the owner's go.
- **2026-09-04 iOS stability before App Store 1.0.1 (#1012 LIVE):** App Store 1.0 is READY_FOR_SALE on build 7 (2026-08-11, ~0 downloads) and is broken twice over — the retired host (#1010) and Season Stats failing to decode `clubs[].team_name: null` from the season-rollup path. #1012 fills rollup club name/logo from Team (positive ids) / ClubProgram (negative ids) with the stored name as fallback, and iOS tolerates a null name ("Club <id>" / "Club"). Verified live: 393195 → Middlesbrough + logo. Fixed build installed on MJ's iPhone; owner check on device is the last gate before bumping to 1.0.1, triggering Xcode Cloud "Default" on main, and submitting.
...
- **2026-09-03 S2 "fans + reach" SHIPPED AND LIVE (session `loanarmy-d9`)** — #978 P0 foundation (s2f1), #983 P1 fans backend, #980 P2 share/card/sitemap/robots, #984 P4 weekly activity email (+ ACA job `job-profile-activity`), #979 P3 web, #986 re-score (**59.8% → 61.0%**), #987/#988 pretty share links. Ledger `ledgers/CONTINUITY_dream-s2.md` (debts + hygiene queue inside). QA walkthrough: https://claude.ai/code/artifact/2c44ca84-b5bd-4b5c-bb5c-6c8697c65373; scorecard: https://claude.ai/code/artifact/1ac9cfc9-9540-4a41-80a6-1e3846fbb8d9.
