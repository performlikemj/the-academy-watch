        ...
        <div className="min-h-screen bg-[#efece2] text-stone-950">
            ...
            <main className="mx-auto grid max-w-6xl gap-6 px-4 py-10 lg:grid-cols-[1fr_340px]">
                <div className="space-y-6">
                    ...
                    {program.updates?.length ? (
                        <Card className="border-stone-200 bg-white/80 shadow-sm">
                            <CardHeader><CardTitle className="font-serif text-2xl">Latest from the program</CardTitle><CardDescription>Approved updates from the program team.</CardDescription></CardHeader>
                            <CardContent className="divide-y divide-stone-200">
                                {program.updates.map((update) => (
                                    <article key={update.id} className="py-5 first:pt-0 last:pb-0">
                                        <div className="flex flex-col justify-between gap-1 sm:flex-row sm:items-baseline"><h3 className="font-serif text-xl font-semibold">{update.title}</h3><time className="text-xs text-stone-500">{update.published_at ? new Date(update.published_at).toLocaleDateString() : null}</time></div>
                                        <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-stone-700">{update.body}</p>
                                        {update.impact ? <p className="mt-3 border-l-2 border-amber-400 pl-3 text-sm font-medium text-stone-700">Impact: {update.impact}</p> : null}
                                    </article>
                                ))}
                            </CardContent>
                        </Card>
                ...
                <aside className="space-y-5">
                    {program.external_support ? (
                        <Card className="border-0 bg-[#f5c95d] text-stone-950 shadow-lg"><CardHeader><div className="flex h-10 w-10 items-center justify-center rounded-full bg-stone-950/10"><Sparkles className="h-5 w-5" /></div><CardTitle className="font-serif text-2xl">Back this program</CardTitle><CardDescription className="text-stone-700">Support happens directly on the program&apos;s approved external page.</CardDescription></CardHeader><CardContent><Button asChild className="w-full rounded-full bg-stone-950 text-white hover:bg-stone-800"><a href={program.external_support.url} target="_blank" rel="noopener noreferrer">Support on {program.external_support.label}<ExternalLink className="ml-2 h-4 w-4" /></a></Button></CardContent></Card>
                    ) : (
                        <Card className="border-0 bg-[#f5c95d] text-stone-950 shadow-lg"><CardHeader><div className="flex h-10 w-10 items-center justify-center rounded-full bg-stone-950/10"><Sparkles className="h-5 w-5" /></div><CardTitle className="font-serif text-2xl">Support is not live yet</CardTitle><CardDescription className="text-stone-700">There is no approved external support link for this program yet.</CardDescription></CardHeader></Card>
                    )}
