...
test('real upload, admin approval and anonymous showcase image', async ({ page, browser, request }) => {
  ...
  expect(served.headers()['cache-control']).toBe('private, max-age=300, must-revalidate')
...
test('approved minor photos render only in authenticated owner and admin views', async ({ page, browser, request }) => {
  test.skip(!fixture.minor_photo_id, 'Requires approved minor fixture')
  await authenticate(page, fixture.owner_token)
  await page.goto(`/local-players/${fixture.minor_player_id}`)
  const image = page.locator('img[src^="blob:"]')
  await expect(image).toBeVisible()
  await expect.poll(() => image.evaluate(img => img.complete && img.naturalWidth > 0)).toBe(true)
  await page.screenshot({ path: `${artifacts}/shots/06-minor-owner-private.png`, fullPage: true })
  const preview = `/api/showcase/media/${fixture.minor_photo_id}/preview`
  expect((await request.get(fixture.minor_public_url)).status()).toBe(404)
  expect((await request.get(preview)).status()).toBe(401)
  expect((await request.get(preview, { headers: { Authorization: `Bearer ${fixture.manager_token}` } })).status()).toBe(404)
  ...
  await admin.getByRole('option', { name: 'Approved', exact: true }).click()
  const row = admin.getByLabel(`Review note for photo ${fixture.minor_photo_id}`).locator('../..')
  const moderatedImage = row.locator('img[src^="blob:"]')
  await expect(moderatedImage).toBeVisible()
  await expect.poll(() => moderatedImage.evaluate(img => img.complete && img.naturalWidth > 0)).toBe(true)
  await admin.screenshot({ path: `${artifacts}/shots/07-minor-admin-private.png`, fullPage: true })
  await adminContext.close()
})
