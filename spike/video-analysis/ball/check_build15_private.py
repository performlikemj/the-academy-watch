...
def fix_path(browser, base):
    with browser.new_context(accept_downloads=True) as ctx:
        ...
        page = open_page(ctx, base + "/ball-truth-review-build15/index.html")
        ...
        settle(page)
        assert page.locator("#legacy-warning").is_hidden()
        fresh = open_page(ctx, base + "/ball-truth-review-build15/index.html")
...
def recovery_salvage(browser):
    ...
    results = {}
    with tempfile.TemporaryDirectory(prefix="ball-build15-recovery-") as folder:
...
def first_open(browser, base):
    ...
    with browser.new_context(accept_downloads=True) as ctx:
        page = ctx.new_page()
        page.goto(base + "/ball-truth-review-build15/index.html")
        settle(page)
        page.evaluate(
...
def keypress_feedback(browser, base):
    """Real build-15 kit in isolated storage: key lights its button, badge shows the saved label."""
    with browser.new_context() as ctx:
        ...
        page.wait_for_function(
            "document.getElementById('none').classList.contains('active')"
        )
        settle(page)
        page.wait_for_function(
            "!document.getElementById('none').classList.contains('active')"
        )
        badge = page.locator("#badge").inner_text()
        assert badge == "No ball · Confirmed", badge
        assert page.evaluate("flashes.none") == 1
        ...
        settle(page)
        select_frame(page, key)
        assert page.locator("#badge").inner_text() == "No ball · Confirmed"
        assert page.evaluate("flashes.none") == 0
        fresh = open_page(ctx, base + "/ball-truth-review-build15/index.html")
        select_frame(fresh, key)
        assert fresh.locator("#badge").inner_text() == "No ball · Confirmed"
        # Button click is the key: same row shape and same feedback on the next queue frame.
        page.locator("#review").click()
        settle(page)
        press(page, "ArrowRight")
        other = page.evaluate("api.key(frames[index])")
        assert other != key
        ...
        settle(page)
        clicked = page.evaluate("key=>labels[key]", other)
        keyed = page.evaluate("key=>labels[key]", key)
        shape = (
            "visible",
            "x",
            "y",
        ...
        assert {k: clicked[k] for k in shape} == {k: keyed[k] for k in shape}
        assert page.locator("#badge").inner_text() == "No ball · Confirmed"
        assert page.evaluate("flashes.none") == 1
        return {
            "key_n_marks_no_ball": True,
            "button_lit": True,
            "badge_after_n": badge,
            "badge_after_reload_and_fresh_tab": "No ball · Confirmed",
            "button_click_equals_key": True,
        }
...
def check(out):
    ...
    try:
        with sync_playwright() as pw:
            ...
            for version in (10, 11, 12, 13, 14):
                with browser.new_context() as ctx:
                    old = open_page(
                        ctx, base + f"/ball-truth-review-build{version}/index.html"
                    )
                    before = snapshot(old)
                    new = open_page(ctx, base + "/ball-truth-review-build15/index.html")
                    ...
                    before = snapshot(old)
                    new = open_page(ctx, base + "/ball-truth-review-build15/index.html")
                    audit[f"build{version}_to_build15"] = {
                        "initial": initial,
            ...
            with browser.new_context(accept_downloads=True) as ctx:
                ...
                result = compare(
                    home / "codex-runs/ball-build15/ball-human-truth-v2-build15.jsonl",
                    exported,
                )
                assert result["verdict"] == "same"
            ...
            audit["keypress_feedback"] = keypress_feedback(browser, base)
            ...
            with browser.new_context() as ctx:
                url = f"http://{address}:{server.server_port}/ball-truth-review-build15/index.html"
