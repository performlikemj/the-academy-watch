...
def fix_path(browser, base):
    with browser.new_context(accept_downloads=True) as ctx:
        ...
        page = open_page(ctx, base + "/ball-truth-review-build13/index.html")
        ...
        settle(page)
        assert page.locator("#legacy-warning").is_hidden()
        fresh = open_page(ctx, base + "/ball-truth-review-build13/index.html")
...
def h1(browser, base, choice):
    with browser.new_context(accept_downloads=True) as ctx:
        ...
        settle(page)
        if choice == "use":
            assert page.evaluate("key=>labels[key].visible", key)
        else:
            assert page.evaluate("key=>labels[key]", key) == mine
        assert page.locator("#legacy-warning").is_hidden()
        page.reload()
        ...
        return {
            ...
            "choice": choice,
            "resolved_across_reload_and_tab": True,
...
def old_clear_case(browser, base, which):
    with browser.new_context(accept_downloads=True) as ctx:
        ...
        settle(page)
        assert page.evaluate("key=>!labels[key]&&state.deleted[key]>0", key)
        assert page.locator("#legacy-warning").is_hidden()
        return {
            "file": which,
            "import_and_reload_preserve_warning": True,
            "use_clears_local_label": True,
        }


def recovery_ab(browser):
    # Synthetic data from the same local test fixture; no MJ rows are copied.
    import re
    import tempfile
    from test_kit_safety import kit as synthetic_kit

    results = {}
    with tempfile.TemporaryDirectory(prefix="ball-build13-recovery-") as folder:
        for version in (12, 13):
            directory = Path(folder) / str(version)
            directory.mkdir()
            kit = synthetic_kit.__wrapped__(directory)
            if version == 12:
                old = (Path.home() / "ball-truth-review-build12/index.html").read_text()
                blocks = re.findall(r"<script>(.*?)</script>", old, re.S)
                html = kit["path"].read_text()
                sources = Path(__file__).parent
                html = html.replace((sources / "truth_io.js").read_text(), blocks[0])
                html = html.replace(
                    (sources / "truth_storage.js").read_text(), blocks[1]
                )
                old_js = (
                    "const api=BallTruth"
                    + blocks[-1].split("const api=BallTruth", 1)[1]
                )
                html = html.replace(
                    (sources / "truth_page.js").read_text(), old_js.rstrip() + "\n"
                )
                kit["path"].write_text(html)
            with browser.new_context(accept_downloads=True) as ctx:
                ...
                settle(page)
                mine = page.evaluate("labels[api.key(frames[188])]")
                ...
                page.on("dialog", lambda d: d.accept())
                upload(page, older)
                count = page.evaluate("Object.keys(labels).length")
                assert count == (188 if version == 12 else 189)
                fresh = open_page(ctx, kit["path"].as_uri())
                assert fresh.evaluate("Object.keys(labels).length") == count
                results[str(version)] = {"labels": count, "fresh_tab_labels": count}
                if version == 13:
                    assert page.evaluate("labels[api.key(frames[188])]") == mine
                    ...
                    upload(page, backup)
                    assert page.evaluate("state") == expected and not page.evaluate(
                        "readOnly"
                    )
                    results["13"]["own_recovery_backup_restores_labels_and_clears"] = (
                        True
                    )
    return results
        ...
        with sync_playwright() as pw:
            ...
            for version in (10, 11, 12):
                with browser.new_context() as ctx:
                    ...
                    before = snapshot(old)
                    new = open_page(ctx, base + "/ball-truth-review-build13/index.html")
                    ...
                    before = snapshot(old)
                    new = open_page(ctx, base + "/ball-truth-review-build13/index.html")
                    audit[f"build{version}_to_build13"] = {
                        "initial": initial,
            ...
            with browser.new_context(accept_downloads=True) as ctx:
                ...
                result = compare(
                    home / "codex-runs/ball-human-truth-v2-build13.jsonl", exported
                )
                assert result["verdict"] == "same"
                audit["zero_edit_export_vs_seed"] = result["counts"]
            audit["real_build8_import"] = fix_path(browser, base)
            audit["H1_use"] = h1(browser, base, "use")
            audit["H1_keep"] = h1(browser, base, "keep")
            for name, which in [
                ("H2a", "current"),
                ("H2b", "pre_clear"),
                ("H3", "own"),
            ...
            audit["recovery_ab"] = recovery_ab(browser)
            browser.close()
            ...
            with browser.new_context() as ctx:
                url = f"http://{address}:{server.server_port}/ball-truth-review-build13/index.html"
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--out",
        type=Path,
        default=Path.home() / "codex-runs/ball-build13-compatibility.json",
    )
    a = p.parse_args()
