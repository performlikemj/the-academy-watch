...
UPDATE alembic_version SET version_num='pm01' WHERE version_num='lp01';
SELECT version_num AS after FROM alembic_version;
SELECT count(*) AS pm01_tables FROM pg_tables WHERE schemaname='public' AND tablename IN ('player_match_entries','showcase_moderation_events');
SQL
