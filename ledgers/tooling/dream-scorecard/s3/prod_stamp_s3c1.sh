...
UPDATE alembic_version SET version_num='s3c1' WHERE version_num='s3b1';
SELECT version_num AS after FROM alembic_version;
SELECT count(*) AS s3c1_tables FROM pg_tables WHERE schemaname='public' AND tablename IN ('club_program_updates');
SQL
