...
UPDATE alembic_version SET version_num='s3d1' WHERE version_num='s3c1';
SELECT version_num AS after FROM alembic_version;
SELECT count(*) AS s3d1_tables FROM pg_tables WHERE schemaname='public' AND tablename IN ('gol_credit_ledger');
SQL
