...
UPDATE alembic_version SET version_num='s4d1' WHERE version_num='s4c1';
SELECT version_num AS after FROM alembic_version;
SELECT count(*) AS s4d1_tables FROM pg_tables WHERE schemaname='public' AND tablename IN ('player_feedback');
SQL
