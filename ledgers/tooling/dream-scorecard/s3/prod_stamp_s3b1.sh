...
UPDATE alembic_version SET version_num='s3b1' WHERE version_num='cb01';
SELECT version_num AS after FROM alembic_version;
SELECT count(*) AS s3b1_tables FROM pg_tables WHERE schemaname='public' AND tablename IN ('stripe_webhook_events','billing_customers','billing_subscriptions','billing_checkout_sessions');
SQL
