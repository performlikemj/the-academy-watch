...
UPDATE alembic_version SET version_num='s4a1' WHERE version_num='s3e1';
SELECT version_num AS after FROM alembic_version;
SELECT count(*) AS s4a1_tables FROM pg_tables WHERE schemaname='public' AND tablename IN ('club_invitations');
SQL
