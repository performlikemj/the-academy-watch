...
UPDATE alembic_version SET version_num='s4c1' WHERE version_num='s4b1';
SELECT version_num AS after FROM alembic_version;
SELECT count(*) AS s4c1_tables FROM pg_tables WHERE schemaname='public' AND tablename IN ('club_results');
SQL
