...
| P2 — Clubs: track, analyze growth, tools to improve | 25 | **66.7%** |
| P3 — Scouts: find and analyze | 20 | **71.4%** |
| P4 — Funding: Patreon/BMAC → club part-ownership | 15 | **46.4%** |
...
| 1.2 | Share about myself | 3 (codex said 2) | LIVE_WEB | [S1 P5; refreshed by astra review 2026-09-05 — the 'must still set the flag' text is stale] trust-tiered auto-approval: allow-listed low-risk fields keep the profile approved for accounts past the configured age with no adverse moderation events; sensitive changes still return the profile to pending (showcase.py:310, :2795). The threshold IS configured in prod (SHOWCASE_TRUST_MIN_ACCOUNT_AGE_DAYS=14, ledgers/CONTINUITY_dream-s1.md:74). Remaining: photos/contract fields still queue for admin; native claim attestation is not a complete bio/photo/reel editor. | Diff fields inside _upsert_subject_showcase_profile, preserving approved low-risk fields while staging contract, club, and agent changes. | M |
...
| 1.7 | Have fans | 3 | LIVE_WEB | Local player pages still have no comments; real fans and a public iOS release remain unproven. | Extend follow_resolver._validate_player and Follow.selector for local_player_id, then expose generic Follow controls on both player pages. | M |
...
### P2 — Clubs: track, analyze growth, tools to improve — 66.7%
...
| 2.3 | Track games without video | 3 (codex said 2) | LIVE_WEB | [S1 P3+P4] clubs record a result and per-player lineup stats without video (POST /api/club/<program>/results) from the MyClub console; rows are club-confirmed and feed the rollup. Remaining: video_match link not persisted; program-scoped uniqueness is app-level until a partial unique index migration. | Add ClubMatch and ClubMatchAppearance keyed to ClubProgram and ClubRosterMember, with manager CRUD for scores, lineups, minutes, goals, and an optional VideoMatch link. | M |
...
| 2.7 | Tools to improve players | 2 (codex said 1) | LIVE_WEB | [astra review 2026-09-05 P2, evidence-adjusted 1→2] Managers now persist per-player coaching briefs and a club system brief (club.py:879, :898) and the console displays analysis checks against them (MyClubConsole.jsx:549) — more than a stub, so 2. Not yet a permissioned development record delivered to the player, and analysis remains dependent on the Film Room workflow (PlayerReel.jsx:158). | Add append-only ClubRosterFeedback keyed to ClubRosterMember, manager CRUD in routes/club.py, recipient reads resolving player_api_id or local_player_id, then MyClub composer and player inbox views. | M |
...
### P3 — Scouts: find and analyze — 71.4%
...
| 3.2 | Find players | 3 | LIVE_WEB_IOS | Remaining gaps are completeness and native presentation of local subjects, not rebuilding discovery (senior-union/graduation limits live under 1.4). | Add local subject support to ScoutWatchlistEntry, Follow, and comparison, then union eligible LocalPlayer rows into _base_scout_query. | L |
...
| 3.5 | Reach out | 3 (codex said 2) | LIVE_WEB_IOS | [adversary-1 up; refreshed by astra review 2026-09-05 — 'tracked players only' is stale] Prod flag is on. Web Send button + IntroduceDialog (ScoutPage.jsx:986-996, :1110; IntroduceDialog.jsx:33) and iOS (PlayerDetailView.swift:259) post contact.py:405. The rail resolves local subjects and verified scouts can initiate requests to eligible signed subjects, with participant-gated messaging on accepted conversations (contact.py:420, :441, :1090, :1125). Remaining: reachable only from the ScoutPage table; scout credential/adult verification gaps remain (review 1.5/3.1). | Smoke-test each routing mode, then encode CONTACT_RAIL_ENABLED=true in the deployed Container App configuration. | S |
| 3.6 | Pay for Scout Pro | 2 (codex said 1) | DESIGNED_ONLY | [astra review 2026-09-05 P3, evidence-adjusted 3→2] Billing is dark in prod: the billing guard hides the whole rail while BILLING_ENABLED is off (billing.py:38) and the ledger records dark production — dark scaffolding is not a launched Scout Pro, and the owner re-scoped the paid offering to prepaid GOL credits (scout_entitlements.py:107). The rail itself is complete and now includes the money-safety fixes the review demanded first (ledgers/DIRECTIVE_money-safety.md 2026-09-05: GOL replay-as-stored-answer, refund settlements that survive event ordering, purchase terms frozen to the Stripe session, orphaned late payments; shipped dark in #1028–#1030, migration s3e1). Becomes 3 at go-live (review B2): Stripe endpoint + live prices + corrected terms copy + BILLING_ENABLED=1 with one real-card purchase/refund acceptance. | Add Stripe Checkout and webhook handling that updates UserAccount.scout_tier, then gate selected Pro features through one entitlement helper. | M |
...
### P4 — Funding: Patreon/BMAC → club part-ownership — 46.4%

| # | Capability | Score | Reach | Blocker | Next step | Effort |
|---|---|---|---|---|---|---|
| 4.1 | Club funding registry / programs | 3 (codex said 2) | LIVE_WEB | [S3 orchestrator verdict] Managers now edit a PENDING ClubProgramProfileRevision (GET/PUT /api/club/<id>/profile, one pending per program, replace-in-place); dual-admin approve/reject with FundingAdminEvent audit; the approved revision changes only via the review route; MyClubConsole editor + AdminFunding queues shipped. No program directory (exact slug only) and prod still has 0 programs — not 4. | Add manager-gated ClubProgramProfileRevision creation through ClubProgramManager, safe-field publication, and a MyClubConsole editor. | M |
...
| 4.4 | External platform connection | 3 (codex said 0) | LIVE_WEB | [S3 orchestrator verdict; reach corrected by astra review 2026-09-05] Moderated external_support {patreon|buy_me_a_coffee} on the revision with strict host/userinfo/port/query/fragment validation and normalisation; only the APPROVED revision reaches /api/programs/<slug>; ProgramPage renders 'Support on <label>' with rel=noopener noreferrer (club.py:189, funding.py:1270, ProgramPage.jsx:164). The review: the recorded MISSING reach contradicted the implementation — it is a live web link-out, not supporter synchronization. Supporter import deferred by design (needs Privacy changes + a real club) — not 4. | Add moderated provider and external_support_url fields to ClubProgramProfileRevision and render a labeled ProgramPage link-out. | M |
...
| 4.6 | Supporter/fan-facing club page | 3 (codex said 2) | LIVE_WEB | [S3 orchestrator verdict] Moderated club_program_updates (pending cap 5, approve sets published_at, withdraw hides) render as 'Latest from the program'; the link-out replaces 'Support is not live yet' only when a link exists; is_fundable still false everywhere (donations gated on counsel). Discoverability unchanged (exact slug) and no real supporters yet — not 4. | Add moderated ClubProgram updates and impact entries through ClubProgramManager and render them on ProgramPage. | M |
...
| 5.4 | Ops and scale | 2 | ADMIN_ONLY | No managed worker deployment or alerts; managed worker operation is unverified; every club processing request still needs an admin. | Deploy vision_worker.main through ACA Service Bus/KEDA, persist chunk checkpoints, schedule maintenance, and alert on queued, stale, and failed VideoAnalysisJob rows. | L |
| 5.5 | Revenue rails | 2 | ADMIN_ONLY | Dark in prod: the billing guard hides the customer and admin endpoints and no live financial acceptance has run (review 5.5); the go-live checklist (review B2 / DIRECTIVE_money-safety.md) — Stripe endpoint, live GOL prices, corrected terms copy, BILLING_ENABLED=1, one real-card purchase + refund — is what returns this to 3. | Create Checkout and signed webhook routes that append VideoCreditLedger purchase rows, then enable the Film Room pricing CTA. | M |
...
|---|---|---|---|---|
| — | Today | — | — | **65.1%** |
| S0 | Unbreak the front door | days | Fix the web claim payload (send contract_status), add the age gate to local self-claims, bridge club claim → console grant without an admin API key, put the digest sender on the nightly job, fix the dead journalist /stripe box. | **65.1%** |
| S1 | One player universe + a games grain | weeks | Let local players into discovery/watchlists/contact/follows (shared subject id), and add a user-entered match row (player or club, provenance-labelled) that feeds the existing season cells. Show it in MyClub as a growth view. | **65.1%** |
| S2 | Fans and reach | weeks | Public follow for any account, fan counts on player pages, shareable player card with per-player og tags and a sitemap, trust-tiered auto-approval so edits stop hiding profiles, email notifications on the events that already exist. | **65.1%** |
| S3 | Money rails | weeks | Stripe Checkout for Scout Pro at the committed price; club bundle subscription; donation checkout with donor-tip model once regulatory scoping is done; club-editable programs; Patreon/BuyMeACoffee link-out plus supporter import as the cheap first bridge. | **65.1%** |
| MS | money-safety (2026-09-05) | done | independent audit 65.1%; 3 P1 + 3 launch blockers fixed; go-live (B2) projected next. | **65.1%** |
| B2 | go-live prepaid GOL (review B2) | next | Owner go-live checklist after the money-safety fixes: Stripe webhook endpoint + live GOL prices/credit envs, corrected prepaid-credit terms (VITE_BILLING_TERMS=1), then BILLING_ENABLED=1 with one real-card purchase + refund accepted; 3.6 and 5.5 return to 3 (review A1+B2, +1.2). | **66.3%** |
| S4 | Film Room self-serve + club tools | weeks–months | Club-triggered processing with a monthly allowance instead of 3-lifetime, worker checkpointing and scheduling, coaching notes and shareable player reports, a manager-scoped club dashboard on the pathway data that already exists. | **68.8%** |
| S5 | Part-ownership | paper first | Counsel decides the vehicle (crowd-equity is securities-regulated; a supporter-membership with governance perks may be the honest v1). Then recurring support tiers and the ownership ledger. | **72.6%** |
| S6 | Ten real participants | ongoing | The multiplier on everything above: pick one cohort (the Forest academy contact or one grassroots club), walk 10 real people through claim → console → intro by hand, instrument the funnel first. | **73.5%** |

After S6 every remaining point is polish to 4s: iOS parity, E2E on the core journeys, tests, the U18 expansion (ROADMAP Phase 5). Reaching 100% means all 32 rows at 4 — that is the finished dream, not a launch bar. A credible launch bar is **S0–S2 done (~65.1%) plus S6 started**.
