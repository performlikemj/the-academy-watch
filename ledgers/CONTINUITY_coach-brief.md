...
- 2026-09-02: `cb01` chains from the moved single head `pm01` (not the directive's former `lp01` head). Six guarded columns, manager routes, roster-name validation, and requested leak coverage implemented; 59 focused tests pass and focused Ruff check/format pass.
...
- Verified: final focused set (C1 + club bridge + migration heads) → 61 passed.
