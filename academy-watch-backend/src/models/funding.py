...
class ClubProgramProfileRevision(db.Model):
    ...
    # Moderated external support link-out (S3-P2a): no money moves through the
    # platform; the URL is allowlisted to patreon.com / buymeacoffee.com hosts
    # by the manager route before it is ever stored or published.
...
class ClubProgramUpdate(db.Model):
    """Club-manager authored program update, public only once admin approved."""
    ...
    program = db.relationship("ClubProgram", backref=db.backref("program_updates", lazy="dynamic"))
    author = db.relationship("UserAccount", foreign_keys=[author_user_id])


def revision_dict(revision: ClubProgramProfileRevision | None) -> dict | None:
    """Shared serializer for club program profile revisions (manager/admin/public)."""
    if revision is None:
        return None
    support = None
...
def update_dict(update: ClubProgramUpdate | None) -> dict | None:
    """Shared serializer for club program updates (manager/admin)."""
    if update is None:
        return None
    return {
        "id": update.id,
