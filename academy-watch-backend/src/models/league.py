...
class Team(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint("team_id", "season", name="uq_team_id_season"),)
...
class PlayerStatsCache(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint("player_api_id", "team_api_id", "season", name="uq_player_stats_cache"),)
...
class Newsletter(db.Model):
    ...
    generated_date = db.Column(db.DateTime, default=lambda: datetime.now(UTC))
    created_at = db.Column(db.DateTime, default=lambda: datetime.now(UTC))
    updated_at = db.Column(db.DateTime, default=lambda: datetime.now(UTC), onupdate=lambda: datetime.now(UTC))
...
class PlayerFlag(db.Model):
    ...
    category = db.Column(
        db.String(30), default="player_data"
    )  # player_data|stats|club_assignment|match_result|missing_data|transfer|other
...
class UserAccount(db.Model):
    ...
    scout_tier = db.Column(db.String(20), nullable=False, default="free", server_default="free")
    ...
    journalist_subscriptions = db.relationship(
    ...
    def to_dict(self):
        return {
            ...
            "last_display_name_change_at": self.last_display_name_change_at.isoformat()
            if self.last_display_name_change_at
            else None,
...
class NewsletterCommentary(db.Model):
    ...
    newsletter_id = db.Column(
        db.Integer, db.ForeignKey("newsletters.id", ondelete="CASCADE"), nullable=True
    )  # Now nullable
    ...
    def to_dict(self):
        ...
        return {
            ...
            if self.team
            else (self.newsletter.team.name if self.newsletter and self.newsletter.team else None),
            "player_id": self.player_id,
...
class CommentaryApplause(db.Model):
    ...
    def sanitize_and_create(
        newsletter_id, author_id, author_name, commentary_type, content, player_id=None, position=0
    ):
...
class JournalistTeamAssignment(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint("user_id", "team_id", name="uq_journalist_team_assignment"),)
...
class JournalistLoanTeamAssignment(db.Model):
    ...
    # Relationships
    journalist = db.relationship(
        "UserAccount", foreign_keys=[user_id], backref=db.backref("loan_team_assignments", lazy=True)
    )
...
class WriterCoverageRequest(db.Model):
    ...
    # Relationships
    writer = db.relationship("UserAccount", foreign_keys=[user_id], backref=db.backref("coverage_requests", lazy=True))
    team = db.relationship("Team", lazy=True)
...
class StripeConnectedAccount(db.Model):
    ...
    # Relationship
    journalist = db.relationship(
        "UserAccount", foreign_keys=[journalist_user_id], backref=db.backref("stripe_account", uselist=False)
    )
...
class StripeSubscriptionPlan(db.Model):
    ...
    # Relationship
    journalist = db.relationship(
        "UserAccount", foreign_keys=[journalist_user_id], backref=db.backref("subscription_plans", lazy=True)
    )
...
class StripeSubscription(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint("subscriber_user_id", "journalist_user_id", name="uq_stripe_subscription"),)

    # Relationships
    subscriber = db.relationship(
        "UserAccount", foreign_keys=[subscriber_user_id], backref=db.backref("stripe_subscriptions", lazy=True)
    )
    journalist = db.relationship(
        "UserAccount", foreign_keys=[journalist_user_id], backref=db.backref("stripe_subscribers", lazy=True)
    )
...
class NewsletterDigestQueue(db.Model):
    ...
    queued_at = db.Column(db.DateTime, default=lambda: datetime.now(UTC), nullable=False)
...
class TeamSubreddit(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint("team_id", "subreddit_name", name="uq_team_subreddit"),)
...
class ContributorProfile(db.Model):
    ...
    created_by = db.relationship(
        "UserAccount", foreign_keys=[created_by_id], backref=db.backref("contributor_profiles", lazy=True)
    )
...
class RebuildConfig(db.Model):
    ...
    logs = db.relationship("RebuildConfigLog", backref="config", lazy="dynamic", cascade="all, delete-orphan")
