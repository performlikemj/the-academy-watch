...
class WeeklyLoanReport(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint("parent_team_id", "week_start_date", "week_end_date", name="uq_weekly_parent_week"),
    )
...
class FixtureTeamStats(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint("fixture_id", "team_api_id", name="uq_fixture_team"),)
...
class FixturePlayerStats(db.Model):
    ...
    def to_dict(self):
        ...
        return {
            ...
            "reds": self.reds,
            "shots": {"total": self.shots_total, "on": self.shots_on},
            "passes": {"total": self.passes_total, "key": self.passes_key, "accuracy": self.passes_accuracy},
            "tackles": {
                "total": self.tackles_total,
                "blocks": self.tackles_blocks,
                "interceptions": self.tackles_interceptions,
            },
            "duels": {"total": self.duels_total, "won": self.duels_won},
            "dribbles": {
                "attempts": self.dribbles_attempts,
                "success": self.dribbles_success,
                "past": self.dribbles_past,
            },
            "fouls": {"drawn": self.fouls_drawn, "committed": self.fouls_committed},
            "penalty": {
...
class WeeklyLoanAppearance(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    weekly_report_id = db.Column(db.Integer, db.ForeignKey("weekly_loan_reports.id"), nullable=False)
    player_api_id = db.Column(db.Integer, nullable=False)
    fixture_id = db.Column(db.Integer, db.ForeignKey("fixtures.id"), nullable=False)
    team_api_id = db.Column(db.Integer, nullable=False)
    ...
    reds = db.Column(db.Integer, default=0)
    __table_args__ = (
        db.UniqueConstraint("weekly_report_id", "player_api_id", "fixture_id", name="uq_week_player_fixture"),
    )
