...
def can_writer_cover_team(user_id: int, team_id: int) -> bool:
    ...
    """
    return JournalistTeamAssignment.query.filter_by(user_id=user_id, team_id=team_id).first() is not None
...
def can_writer_cover_player(user_id: int, player_id: int, team_id: int = None) -> bool:
    ...
    if tracked:
        assignment = JournalistTeamAssignment.query.filter_by(user_id=user_id, team_id=tracked.team_id).first()
        if assignment:
            return True

    loan = TrackedPlayer.query.filter_by(player_api_id=player_id).order_by(TrackedPlayer.updated_at.desc()).first()
    ...
    if loan.team_id:
        parent_assignment = JournalistTeamAssignment.query.filter_by(user_id=user_id, team_id=loan.team_id).first()
        if parent_assignment:
            return True

    # Also check if assigned to the team_id passed (handles cross-season team IDs)
    if team_id and team_id != loan.team_id:
        team_assignment = JournalistTeamAssignment.query.filter_by(user_id=user_id, team_id=team_id).first()
        if team_assignment:
            return True
    ...
    if loan.current_club_db_id:
        loan_team_assignment = JournalistLoanTeamAssignment.query.filter_by(
            user_id=user_id, loan_team_id=loan.current_club_db_id
        ).first()
    ...
    if loan.current_club_name:
        ...
        loan_name_assignment = JournalistLoanTeamAssignment.query.filter(
            JournalistLoanTeamAssignment.user_id == user_id, JournalistLoanTeamAssignment.loan_team_name.in_(variations)
        ).first()
...
def get_writer_available_players(user_id: int) -> list:
    ...
    players = (
        TrackedPlayer.query.filter(TrackedPlayer.is_active, or_(*conditions))
        .order_by(TrackedPlayer.player_name.asc())
        .all()
    )
...
def subscribe_to_journalist(journalist_id):
    ...
    try:
        ...
        subscription = JournalistSubscription.query.filter_by(
            subscriber_user_id=subscriber.id, journalist_user_id=journalist.id
        ).first()
        ...
        new_subscription = JournalistSubscription(
            subscriber_user_id=subscriber.id, journalist_user_id=journalist.id, is_active=True
        )
...
def unsubscribe_from_journalist(journalist_id):
    ...
    try:
        ...
        subscription = JournalistSubscription.query.filter_by(
            subscriber_user_id=subscriber.id, journalist_user_id=journalist_id
        ).first()
    ...
    try:
        ...
        if user_id:
            ...
            sub = JournalistSubscription.query.filter_by(
                subscriber_user_id=user_id, journalist_user_id=journalist_id, is_active=True
            ).first()
            if sub:
                is_subscribed = True

        articles = (
            NewsletterCommentary.query.filter_by(author_id=journalist_id, is_active=True)
...
def search_commentaries():
    ...
    try:
        ...
        query = query.outerjoin(UserAccount, NewsletterCommentary.author_id == UserAccount.id).outerjoin(
        ...
        results = query.filter(search_filter).order_by(NewsletterCommentary.created_at.desc()).limit(10).all()
...
def _get_player_week_stats(player_id: int, week_start, week_end, current_club_api_id: int | None = None) -> list:
    ...
    try:
        ...
        if current_club_api_id:
            stats_query = stats_query.filter(FixturePlayerStats.team_api_id == current_club_api_id)
...
def _get_season_stats(player_id: int, season: int = None, current_club_api_id: int | None = None) -> list:
    ...
    try:
        ...
        if date_range == "week":
            ...
            if week_start and week_end:
                try:
                    start_date = (
                        datetime.fromisoformat(week_start).date() if isinstance(week_start, str) else week_start
                    )
                    end_date = datetime.fromisoformat(week_end).date() if isinstance(week_end, str) else week_end
                    fixtures_data = _get_player_week_stats(
...
def get_commentary_public(commentary_id: int):
    ...
    try:
        ...
        if user:
            sub = JournalistSubscription.query.filter_by(
                subscriber_user_id=user.id, journalist_user_id=commentary.author_id, is_active=True
            ).first()
        ...
        if commentary.player_id:
            ...
            lp = (
                TrackedPlayer.query.filter_by(player_api_id=commentary.player_id)
                .order_by(TrackedPlayer.updated_at.desc())
                .first()
            )
            ...
            if commentary.week_start_date and commentary.week_end_date:
                week_stats = _get_player_week_stats(
                    commentary.player_id, commentary.week_start_date, commentary.week_end_date
                )
...
def get_my_subscriptions():
    ...
    try:
        ...
        subs = JournalistSubscription.query.filter_by(subscriber_user_id=user.id, is_active=True).all()
...
def get_loan_destinations():
    ...
    try:
        ...
        results = (
            ...
            .group_by(TrackedPlayer.current_club_name, TrackedPlayer.current_club_db_id)
            .order_by(func.count(TrackedPlayer.id).desc())
            .all()
        )
...
def get_writer_available_players_endpoint():
    ...
    try:
        ...
        result = {"players": [], "by_parent_club": {}, "by_loan_team": {}}
...
def get_writer_commentaries():
    ...
    try:
        ...
        commentaries = (
            NewsletterCommentary.query.filter_by(author_id=user.id)
            .order_by(NewsletterCommentary.created_at.desc())
            .all()
        )
...
def _render_chart_block_to_html(
    block: dict, player_id: int = None, week_start: str = None, week_end: str = None
) -> str:
...
def _fetch_chart_data_for_rendering(
    player_id: int, chart_type: str, stat_keys: list, date_range: str, week_start: str = None, week_end: str = None
) -> dict:
    ...
    try:
        ...
        if commentary_id:
            ...
            if not can_edit and (user.is_editor or _is_admin_request()):
                ...
                if commentary_author and commentary_author.managed_by_user_id == user.id or _is_admin_request():
                    can_edit = True
            if not can_edit:
            ...
            if "week_start_date" in data:
                ...
                commentary.week_start_date = (
                    date.fromisoformat(data["week_start_date"]) if data["week_start_date"] else None
                )
            ...
            if "contributor_id" in data:
                ...
                if contributor_id == "" or contributor_id is None:
                    ...
                    contributor = ContributorProfile.query.filter_by(
                        id=contributor_id, created_by_id=user.id, is_active=True
                    ).first()
                    if not contributor:
        ...
        if contributor_id:
            ...
            contributor = ContributorProfile.query.filter_by(
                id=contributor_id, created_by_id=author.id, is_active=True
            ).first()
            if not contributor:
                return jsonify({"error": "Contributor not found or not owned by you"}), 404
...
def submit_coverage_request():
    ...
    try:
        ...
        existing = WriterCoverageRequest.query.filter_by(
            user_id=user.id, coverage_type=coverage_type, team_name=team_name, status="pending"
        ).first()
        ...
        if coverage_type == "parent_club" and team_id:
            existing_assignment = JournalistTeamAssignment.query.filter_by(user_id=user.id, team_id=team_id).first()
            ...
            existing_loan = JournalistLoanTeamAssignment.query.filter_by(
                user_id=user.id, loan_team_name=team_name
            ).first()
            if existing_loan:
...
def get_player_stats(player_id):
    ...
    try:
        ...
        if not loaned:
            loaned = (
                TrackedPlayer.query.filter_by(player_api_id=player_id).order_by(TrackedPlayer.updated_at.desc()).first()
            )
...
def get_journalist_own_stats():
    ...
    try:
        ...
        total_subscribers = JournalistSubscription.query.filter_by(journalist_user_id=user.id, is_active=True).count()
        ...
        subscribers_last_7_days = JournalistSubscription.query.filter(
            ...
            JournalistSubscription.is_active,
        ...
        all_subs = (
            JournalistSubscription.query.filter(
                JournalistSubscription.journalist_user_id == user.id,
                JournalistSubscription.created_at >= ninety_days_ago,
            )
            .order_by(JournalistSubscription.created_at.asc())
            .all()
        )
        ...
        timeline_data = [{"week": k, **v} for k, v in sorted(timeline.items())]
...
def _get_journalists_with_writeups_for_newsletter(newsletter, current_user_id=None):
    ...
    base_query = NewsletterCommentary.query.filter(NewsletterCommentary.is_active.is_(True))
    ...
    if api_team_id:
        conditions.append(
            and_(
                ...
                NewsletterCommentary.team_id.in_(db.session.query(Team.id).filter(Team.team_id == api_team_id)),
            )
        )
...
def get_newsletter_journalist_view(newsletter_id):
    ...
    try:
        ...
        if journalist_ids_param:
            try:
                requested_journalist_ids = {
                    int(x.strip()) for x in journalist_ids_param.split(",") if x.strip().isdigit()
                }
            except ValueError:
                pass

        # Get available journalists for this newsletter
        available_journalists = _get_journalists_with_writeups_for_newsletter(newsletter, current_user_id)
        ...
        commentaries = {"intro": [], "player": {}, "summary": []}
        ...
        try:
            ...
            if isinstance(content_obj, dict):
                ...
                if isinstance(rendered, dict):
                    newsletter_data["rendered"] = {k: (v if isinstance(v, str) else "") for k, v in rendered.items()}
                ...
                if isinstance(raw_sections, list):
                    for sec in raw_sections:
                        if not isinstance(sec, dict):
                            continue
                        section_data = {"title": sec.get("title", ""), "players": []}
                        items = sec.get("items", [])
        ...
        for player_id, player_comms in commentaries["player"].items():
            ...
            enriched_player_commentaries[player_id] = {"player_info": player_info, "commentaries": player_comms}
...
def get_journalist_public_stats(journalist_id):
    ...
    try:
        ...
        total_subscribers = JournalistSubscription.query.filter_by(
            journalist_user_id=journalist_id, is_active=True
        ).count()
        ...
        return jsonify(
            {"journalist_id": journalist_id, "total_subscribers": total_subscribers, "team_breakdown": team_stats}
        )
    ...
    try:
        ...
        for journalist in journalists:
            ...
            subscriber_count = JournalistSubscription.query.filter_by(
                journalist_user_id=journalist.id, is_active=True
            ).count()
    ...
    try:
        ...
        if coverage_request.coverage_type == "parent_club":
            ...
            existing = JournalistTeamAssignment.query.filter_by(
                user_id=coverage_request.user_id, team_id=coverage_request.team_id
            ).first()
            ...
            existing = JournalistLoanTeamAssignment.query.filter_by(
                user_id=coverage_request.user_id, loan_team_name=coverage_request.team_name
            ).first()
        ...
        coverage_request.reviewed_at = datetime.now(UTC)
...
def admin_assign_loan_teams(journalist_id):
    ...
    try:
        ...
        for lt in loan_teams:
            ...
            assignment = JournalistLoanTeamAssignment(
                user_id=journalist.id, loan_team_id=team_id, loan_team_name=team_name, assigned_by=None
            )
            db.session.add(assignment)
...
def list_manual_submissions():
    ...
    try:
        user = g.user
        submissions = (
            ManualPlayerSubmission.query.filter_by(user_id=user.id)
            .order_by(ManualPlayerSubmission.created_at.desc())
            .all()
        )
...
def list_managed_writers():
    ...
    try:
        ...
        if is_admin:
            ...
            writers = (
                UserAccount.query.filter(UserAccount.managed_by_user_id.isnot(None))
            ...
            writers = (
                UserAccount.query.filter_by(managed_by_user_id=editor.id).order_by(UserAccount.created_at.desc()).all()
            )
...
def update_placeholder_writer(writer_id):
    ...
    try:
        ...
        writer.updated_at = datetime.now(UTC)
        db.session.commit()
...
def editor_assign_teams(writer_id):
    ...
    try:
        ...
        for tid in team_ids:
            ...
            if latest_team_id:
                assign = JournalistTeamAssignment(user_id=writer.id, team_id=latest_team_id, assigned_by=editor.id)
                db.session.add(assign)
                assignments.append(assign)
...
def send_claim_invite(writer_id):
    ...
    try:
        ...
        writer.claim_token_expires_at = datetime.now(UTC) + timedelta(hours=24)
...
def get_writer_contributors():
    ...
    try:
        ...
        contributors = (
            ContributorProfile.query.filter_by(created_by_id=user.id, is_active=True)
            .order_by(ContributorProfile.name)
            .all()
        )
...
def update_contributor(contributor_id):
    ...
    try:
        ...
        if "attribution_url" in data:
            attribution_url = (data.get("attribution_url") or "").strip() or None
            if attribution_url and not (
                attribution_url.startswith("http://") or attribution_url.startswith("https://")
            ):
                return jsonify({"error": "Attribution URL must start with http:// or https://"}), 400
