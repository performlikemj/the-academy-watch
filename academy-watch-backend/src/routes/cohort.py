...
from src.models.league import RebuildConfig, RebuildConfigLog, db
    ...
    try:
        ...
        for page in range(1, 11):
            APICache.invalidate_cached(
                "players",
                {
                    "team": int(team_api_id),
                    "league": int(league_api_id),
                    "season": int(season),
                    "page": page,
                },
            )
...
def admin_cohort_seed_status():
    ...
    cohorts = AcademyCohort.query.order_by(AcademyCohort.season.desc(), AcademyCohort.team_name).all()
    return jsonify(
        {
...
def admin_re_derive_statuses():
    ...
    query = CohortMember.query.filter(CohortMember.journey_synced)
...
def get_cohort(cohort_id):
    ...
    if include_members and data.get("members"):
        ...
        sold_api_ids = [m["player_api_id"] for m in data["members"] if m.get("current", {}).get("status") == "sold"]
        if sold_api_ids:
            fees = {
                tp.player_api_id: tp.sale_fee
...
def cohort_teams():
    ...
    results = (
        ...
        .group_by(AcademyCohort.team_api_id)
        .order_by(db.func.max(AcademyCohort.team_name))
        .all()
    )

    # Resolve correct parent club logos from TeamProfile
    team_ids = [r.team_api_id for r in results]
    profiles = {p.team_id: p.logo_url for p in TeamProfile.query.filter(TeamProfile.team_id.in_(team_ids)).all()}
...
def _get_default_config():
    ...
    from src.services.big6_seeding_service import BIG_6, COHORT_DISCOVER_TIMEOUT, PLAYER_SYNC_TIMEOUT, SEASONS
...
def admin_get_rebuild_config(config_id):
    ...
    recent_logs = config.logs.order_by(RebuildConfigLog.created_at.desc()).limit(20).all()
...
def admin_rebuild_config_history(config_id):
    ...
    config = RebuildConfig.query.get_or_404(config_id)
    logs = config.logs.order_by(RebuildConfigLog.created_at.desc()).all()
