...
class APIFootballClient:
    ...
    def __init__(self, api_key: str | None = None):
        ...
        self.assume_full_minutes_if_started = os.getenv("ASSUME_FULL_MINUTES_IF_STARTED", "false").lower() in (
            "true",
            "1",
            "yes",
        )
        ...
        self.current_date = datetime.now(UTC).date()  # Keep for loan data generation
    ...
    def _check_quota_limit(self) -> None:
        ...
        try:
            ...
            if current >= limit:
                raise RuntimeError(
                    f"API-Football daily quota reached ({current}/{limit}). No more live API calls will be made today."
                )
        except ImportError:
            pass
    ...
    def _get_sample_data(self, endpoint: str, params: dict[str, Any] = None) -> dict[str, Any]:
        ...
        if endpoint == "leagues":
                ...
                "response": [
                    {
                        ...
                        "seasons": [
                            {
                                "year": self.current_season_start_year,
                                "start": self.season_start_date.strftime("%Y-%m-%d"),
                                "end": self.season_end_date.strftime("%Y-%m-%d"),
                                "current": True,
                            }
                        ],
                    }
            ...
            return {
                "response": [
                    {
                        "player": {"id": 1001, "name": "Mason Greenwood"},
                    ...
                    {
                        "player": {"id": 1002, "name": "Jadon Sancho"},
                        "update": current_datetime_str,
                        "transfers": [
                            {
                    ...
                    {
                        "player": {"id": 1003, "name": "Joao Felix"},
                        "update": current_datetime_str,
                        "transfers": [
                            {
                    ...
                    {
                        "player": {"id": 1005, "name": "Conor Gallagher"},
                        "update": current_datetime_str,
                        "transfers": [
                            {
                    ...
                    {
                        "player": {"id": 1006, "name": "Folarin Balogun"},
                        "update": current_datetime_str,
                        "transfers": [
                            {
    ...
    def get_fixtures_for_team(self, team_id: int, season: int, start: str, end: str) -> list[dict[str, Any]]:
        ...
        try:
            resp = self._make_request(
                "fixtures", {"team": normalized_team_id, "season": season, "from": start, "to": end}
            )
    ...
    def get_fixtures_for_team_cached(
        ...
        try:
            ...
            api_ids = {(fx.get("fixture") or {}).get("id") for fx in api_fixtures}
    ...
    def get_player_stats_for_fixture(
        self, player_id: int, season: int, fixture_id: int, fixture_obj: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        """
        ...
        if stats and stats.get("statistics"):
            ...
            if (
                s.get("shots", {}).get("total") is not None
                or s.get("passes", {}).get("total") is not None
                or s.get("rating") is not None
            ):
                ...
                if player_name and match_date:
                    ...
                    if external_stats:
                        ...
                        if external_stats.get("shots_total") is not None:
                            st_entry["shots"] = {
                                "total": external_stats["shots_total"],
                                "on": external_stats.get("shots_on"),
                            }
                        if external_stats.get("passes_total") is not None:
                            st_entry["passes"] = {
                                "total": external_stats["passes_total"],
                                "key": external_stats.get("passes_key"),
                            }
                        ...
                        if external_stats.get("duels_total") is not None:
                            st_entry["duels"] = {
                                "total": external_stats["duels_total"],
                                "won": external_stats.get("duels_won"),
                            }
                        if external_stats.get("dribbles_attempts") is not None:
                            st_entry["dribbles"] = {
                                "attempts": external_stats["dribbles_attempts"],
                                "success": external_stats.get("dribbles_success"),
                            }
    ...
    def _get_player_stats_from_lineups_and_events(self, player_id: int, fixture_id: int) -> dict[str, Any]:
        ...
        if events:
            ...
            if goals_total == 0:
                goals_total = sum(
                    1 for ev in events if ev.get("type") == "Goal" and (ev.get("player") or {}).get("id") == player_id
                )
            # Only add assists from events if lineup didn't already include them
            if assists_total == 0:
                assists_total = sum(
                    1 for ev in events if ev.get("type") == "Goal" and (ev.get("assist") or {}).get("id") == player_id
                )
            ...
            if reds == 0:
                reds = sum(
                    ...
                    and (ev.get("player") or {}).get("id") == player_id
                )
    ...
    def verify_player_id_via_fixtures(
        self, candidate_player_id: int, player_name: str, loan_team_id: int, season: int, max_fixtures: int = 5
    ) -> tuple[int, str | None]:
        """
    ...
    def get_player_limited_stats(
        self, player_id: int, player_name: str, team_api_id: int, season: int, max_fixtures: int = 50
    ) -> dict:
        """
        ...
        try:
            ...
            fixtures_resp = self._make_request(
                "fixtures", {"team": team_api_id, "season": season, "status": "FT", "last": max_fixtures}
            )
            ...
            for fixture in fixtures:
                ...
                for event in events:
                    ...
                    player_match = event_player.get("id") == player_id or _names_match(
                        event_player.get("name", ""), player_name
                    )
                    assist_match = event_assist.get("id") == player_id or _names_match(
                        event_assist.get("name", ""), player_name
                    )
    ...
    def summarize_loanee_week(
        ...
        *,
        ...
        normalized_team_id: int | None = None
            ...
            if db_session:
                ...
                if db_fixture:
                    ...
                    player_stats_row = (
                        ...
                        .filter_by(fixture_id=db_fixture.id, player_api_id=player_id)
                        .first()
                    )
            ...
            if db_session:
                ...
                fixtures = (
                    ...
                    .filter(
                        ...
                        or_(Fixture.home_team_api_id == loan_team_id, Fixture.away_team_api_id == loan_team_id),
                    )
                            ...
                            if db_session:
                                try:
                                    ...
                                    if tp_row:
                                        ...
                                        tp_row.notes = (
                                            (tp_row.notes or "")
                                            + f" | ID auto-corrected pre-newsletter: {info['player_api_id']} → {verified_id}"
                                        )
                                        db_session.commit()
                                        logger.info("✅ Updated TrackedPlayer ID in database")
                                        ghost_deleted = (
                                            db_session.query(FixturePlayerStats)
                                            .filter(
    ...
    def get_current_loans_for_team(self, team_id: int) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for transfer in transfers:
                ...
                current_window = (
                    f"{self.current_season_start_year}-{str(self.current_season_start_year + 1)[-2:]}::FULL"
                )
    ...
    def _iter_loan_transfers(self, player_block: dict, window_key: str, season_year: int = None):
        ...
        for t in player_block.get("transfers", []):
            ...
            if not transfer_date:
                logger.debug("  Skipping transfer with no date")
                continue
                ...
                if transfer_blocks:
                    ...
                    for tblock in transfer_blocks:
                        ...
                        for loan_data in self._iter_loan_transfers(tblock, window_key, season_year):
                            ...
                            if player_id not in out or loan_data["transfer_date"] > out[player_id].get(
                                "transfer_date", ""
                            ):
    ...
    def detect_multi_team_players(
        self, league_ids: list[int] = None, window_key: str = None, season: int = None
    ) -> dict[int, list[int]]:
        """
    ...
    def analyze_transfer_type(
        self, player_id: int, multi_team_dict: dict[int, list[int]] = None, window_key: str = None, season: int = None
    ) -> dict[str, Any]:
        ...
        try:
            ...
            if permanent_transfers_in_window > loans_in_window:
                loan_indicators.append(
                    f"More permanent transfers ({permanent_transfers_in_window}) than loans ({loans_in_window})"
                )
                loan_confidence -= 0.6
            ...
            while True:
                ...
                for entry in players_data:
                    ...
                    pname = (
                        ...
                        or ((player_info.get("firstname") or "") + " " + (player_info.get("lastname") or "")).strip()
                    )
                    if pname:
        ...
        sample_transfers = [
            {
                "player": {
                    ...
                    "lastname": "Greenwood",
                    "age": 22,
                    "birth": {"date": "2001-10-01", "place": "Bradford", "country": "England"},
                    "nationality": "England",
                    "height": "181 cm",
                    "weight": "76 kg",
                    "injured": False,
            ...
            },
            {
                "player": {
                    ...
                    "lastname": "Sancho",
                    "age": 24,
                    "birth": {"date": "2000-03-25", "place": "London", "country": "England"},
                    "nationality": "England",
                    "height": "180 cm",
                    "weight": "76 kg",
                    "injured": False,
        ...
        if team_id:
            ...
            for transfer_item in sample_transfers:
                for transfer in transfer_item["transfers"]:
                    ...
                    if teams.get("out", {}).get("id") == team_id or teams.get("in", {}).get("id") == team_id:
    ...
    def _get_sample_team_players(self, team_id: int, season: int) -> list[dict[str, Any]]:
        ...
        players_data = team_player_mapping.get(
            team_id,
            [
                {"id": 9001, "name": "Player A", "position": "Midfielder"},
                {"id": 9002, "name": "Player B", "position": "Attacker"},
                {"id": 9003, "name": "Player C", "position": "Defender"},
            ],
        )
            ...
            sample_players.append(
                {
                    "player": {
                        ...
                        "age": 22,
                        "birth": {"date": "2002-01-01", "place": "Unknown", "country": "Unknown"},
                        "nationality": "Unknown",
                        "height": "180 cm",
                        "weight": "75 kg",
                        "injured": False,
                    ...
                    "statistics": [
                        {"team": {"id": team_id, "name": "Team Name"}, "games": {"position": player_data["position"]}}
                    ],
                }
            )
    ...
    def _get_sample_player_data(self, player_id: int) -> dict[str, Any]:
        ...
        sample_players = {
            1001: {
                ...
                "statistics": [{"team": {"id": 33, "name": "Manchester United"}, "games": {"position": "Attacker"}}],
            },
            1002: {
                "player": {
                    "id": 1002,
        ...
        if player_id in sample_players:
            ...
            return {
                "player": {
                    ...
                    "age": 25,
                    "birth": {"date": "1999-01-01", "place": "Unknown", "country": "Unknown"},
                    "nationality": "Unknown",
                    "height": "180 cm",
                    "weight": "75 kg",
                    "injured": False,
                ...
                "statistics": [{"team": {"id": 0, "name": "Unknown Team"}, "games": {"position": "Unknown"}}],
            }
    ...
    def batch_get_player_transfers(
        self, player_ids: list[int], max_workers: int = 5, rate_limit_delay: float = 0.1
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # Submit all tasks
            futures = {executor.submit(fetch_with_delay, pid): pid for pid in player_ids}
    ...
    def detect_incremental_loans(
        self, window_key: str, last_check: datetime, league_ids: list[int] = None
    ...
    def clear_transfer_cache(self, player_id: int | None = None):
        """
    ...
    def get_cache_stats(self) -> dict[str, Any]:
        ...
        transfer_expired = sum(
            1 for _, (_, timestamp) in self._transfer_cache.items() if now - timestamp >= self._transfer_cache_ttl
        )

        stats_expired = sum(
            1 for _, (_, timestamp) in self._stats_cache.items() if now - timestamp >= self._stats_cache_ttl
        )
