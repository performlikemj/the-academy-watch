...
def get_result_emoji(result: str | None) -> str:
...
def format_stats_line(stats: dict | None) -> str:
...
def format_expanded_stats(stats: dict | None) -> str:
    ...
    position = stats.get("position", "") or ""
    has_gk = position in ("Goalkeeper", "G") and (
        stats.get("saves") is not None or stats.get("goals_conceded") is not None
    )
...
def format_matches(matches: list | None, upcoming_fixtures: list | None) -> str:
...
def convert_newsletter_to_markdown(
    newsletter: dict, include_expanded_stats: bool = True, include_links: bool = True, web_url: str | None = None
) -> str:
    ...
    lines.append("")
    lines.append(
        "*Generated by [The Academy Watch](https://theacademywatch.com) — Weekly loan watch newsletters for football fans*"
    )
