...
def _apply_player_lookup(
    content_json: dict, lookup: dict[str, list[dict[str, Any]]] | None = None
) -> tuple[dict, bool]:
...
def build_enhanced_queries(player: str, loan_team: str, opponent: str, competition: str, league_lang: str) -> list:
    ...
    if league_lang == "es":
        ...
        base_terms.extend(["Analyse", "Bewertung", "Fan-Reaktionen", "Spielbericht", "Spielerleistung", "Fan-Meinung"])
    elif league_lang == "it":
        base_terms.extend(
            [
                "analisi",
    ...
    queries.extend([f'"{player}" {loan_team} {term}' for term in ["news", "latest", "update", "performance"]])
...
def _filter_hits_for_player(cat: dict, player_name: str, loan_team: str, week_start: date, week_end: date) -> dict:
    ...
    def ok_item(r: dict) -> bool:
        ...
        d = _parse_any_date(
            r.get("date") or r.get("published") or r.get("published_date") or r.get("age") or r.get("page_age")
        )
...
DateStr = Annotated[str, StringConstraints(pattern=r"^\d{4}-\d{2}-\d{2}$", min_length=10, max_length=10)]
...
async def persist_newsletter(ctx, args) -> dict[str, Any]:
    ...
    def _valid_content(payload: dict) -> bool:
        return (
            isinstance(payload, dict) and payload.get("title") is not None and isinstance(payload.get("sections"), list)
        )
    ...
    if not _valid_content(content_json):
        ...
        content_json = {
            "title": content_json.get("title") or "Weekly Loan Update",
...
async def generate_weekly_newsletter(team_db_id: int, target_date: date, force_refresh: bool = False) -> dict[str, Any]:
    ...
    report = await fetch_weekly_report(
        ctx=None,
        args={
...
def _persist_team_profile(payload: dict | None) -> TeamProfile | None:
    ...
    try:
        ...
        if record is None:
            record = profile_model(team_id=team_id, created_at=datetime.now(UTC))
            session.add(record)
...
def _team_logo_for_player(player_id: Any, loan_team_name: str | None = None) -> str | None:
    ...
    try:
        tp = (
            ...
            .filter(TrackedPlayer.player_api_id == pid, TrackedPlayer.is_active)
    ...
    if team_api_id is None and loan_team_name:
        try:
            team_row = (
                db.session.query(Team).filter(Team.name == loan_team_name).order_by(Team.updated_at.desc()).first()
            )
...
def _score_player(item: dict) -> float:
    s = item.get("stats", {}) or {}
    return (
        5 * (s.get("goals", 0) or 0)
        + 3 * (s.get("assists", 0) or 0)
        + (s.get("minutes", 0) or 0) / 90.0
        - 1 * (s.get("yellows", 0) or 0)
        - 3 * (s.get("reds", 0) or 0)
    )
...
def lint_and_enrich(news: dict) -> dict:
    ...
    if player_ids:
        try:
            ...
            sofascore_lookup = {
                int(rec.player_id): int(rec.sofascore_id) for rec in records if getattr(rec, "sofascore_id", None)
            }
        except Exception:
            sofascore_lookup = {}
    ...
    news["by_numbers"] = {
        "minutes_leaders": [
            {"player": _display_name(i.get("player_name")), "minutes": _stat(i, "minutes")} for i in mins_leaders
        ],
        "ga_leaders": [
...
def _inject_links_from_search_context(
    content_json: dict, search_context: dict | None, max_links: int = 3
) -> tuple[dict, bool, int]:
...
def _render_variants_custom(
    news: dict, team_name: str | None, commentaries: list, use_snippets: bool = False, render_mode: str = "web"
) -> dict:
    """
