...
def _backfill_player_stats_cache_legacy(dry_run=False):
    ...
    if not dry_run:
        db.session.commit()
        log.info(f"[stats_cache] Created/updated {created} PlayerStatsCache rows, skipped {skipped} (no loan team)")
    else:
        db.session.rollback()
        log.info(f"[stats_cache] DRY RUN — would create/update {created} rows, skip {skipped}")
