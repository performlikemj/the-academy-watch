...
def _build_helpers(dataframes: dict) -> dict:
    ...
    def _dedup_tracked(df):
        ...
        out = (
            out.sort_values(["_prio", "updated_at"], ascending=[True, False])
    ...
    def first_team_graduates(team_name=None):
        ...
        return (
            merged[["player_name", "name", "position", "nationality", "age"]]
    ...
    def active_academy_pipeline(team_name=None):
        ...
        active = _dedup_tracked(tracked[tracked["status"].isin(["academy", "on_loan", "first_team"])])
        ...
        if not fixture_stats.empty:
            ...
            team_mode = fs.groupby("player_api_id")["team_api_id"].agg(
            ...
            pos_mode = fs.groupby("player_api_id")["position"].agg(
                lambda x: x.mode().iloc[0] if not x.dropna().mode().empty else None
            )
        ...
        return (
            merged[["player_api_id", "player_name", "name", "status", "position", "current_club", "age"]]
            .rename(columns={"name": "team"})
    ...
    def academy_first_team_apps(team_name=None):
        ...
        result = (
            ft.groupby("name")
            .agg(total_graduates=("player_api_id", "count"), total_first_team_apps=("total_first_team_apps", "sum"))
            .reset_index()
            .rename(columns={"name": "team"})
        )
    ...
    def top_loan_performers(season=None, limit=20):
        ...
        if tracked.empty or fixture_stats.empty:
            return pd.DataFrame(
                columns=["player_name", "parent_club", "loan_club", "goals", "assists", "minutes", "avg_rating"]
            )
        ...
        agg = (
            merged.groupby(["player_api_id", "player_name", "parent_club", "loan_club"])
            .agg(
                goals=("goals", "sum"),
                assists=("assists", "sum"),
                minutes=("minutes", "sum"),
        ...
        return agg[
            ["player_api_id", "player_name", "parent_club", "loan_club", "goals", "assists", "minutes", "avg_rating"]
        ].reset_index(drop=True)
    ...
    def player_career(player_name):
        ...
        return entries[
            ["player_api_id", "season", "club_name", "level", "appearances", "goals", "assists", "minutes"]
        ].reset_index(drop=True)
    ...
    def _find_similar_via_journey(target_pid, per90_fixture, season, position, limit):
        ...
        if not fs.empty:
            ...
            result["team"] = (
                result["player_api_id"]
                .map(team_mode)
                .apply(lambda x: _resolve_team_name(x) if pd.notna(x) else "Unknown")
            )
        else:
            result["team"] = "Unknown"
    ...
    def find_similar_players(player_name, position=None, season=None, limit=10, min_minutes=450):
        ...
        result["team"] = (
            result["player_api_id"].map(team_mode).apply(lambda x: _resolve_team_name(x) if pd.notna(x) else "Unknown")
        )
    ...
    def find_hidden_talent(position=None, season=None, limit=15, min_minutes=450, loan_only=True):
        ...
        if "competition_name" in fs.columns:
            league_mode = fs.groupby("player_api_id")["competition_name"].agg(
                lambda x: x.mode().iloc[0] if not x.mode().empty else None
            )
    ...
    def suggest_loan_destinations(player_name, season=None, limit=5):
        ...
        per90_player = _compute_per90_stats(fixture_stats, season=season, min_minutes=90, player_ids=[target_pid])
        ...
        player_is_creative = player_stats.get("key_passes_p90", 0) > 1.0 or player_stats.get("assists_p90", 0) > 0.15
        player_is_physical = player_stats.get("duels_p90", 0) > 5.0 or player_stats.get("tackles_p90", 0) > 2.0
