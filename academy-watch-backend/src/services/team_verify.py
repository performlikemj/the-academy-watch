...
def audit_team_consistency(team_db_id: int) -> Dict[str, Any]:
    ...
    parent_local_league_id: Optional[int] = None
    parent_local_league_name: Optional[str] = None
    if team.league_id:
    ...
    rows = (
        TrackedPlayer.query
        .filter_by(team_id=team_db_id, is_active=True)
    ...
    *,
    ...
    if not dry_run:
        ...
        try:
            ...
            logger.exception(
                'team_verify: refresh_and_heal failed for team %s', team_db_id
            )
