...
def queue_newsletter_for_digest(user_id: int, newsletter_id: int) -> bool:
    ...
    try:
        ...
        queue_entry = NewsletterDigestQueue(
            user_id=user_id, newsletter_id=newsletter_id, week_key=week_key, queued_at=datetime.now(UTC), sent=False
        )
...
def send_digest_emails(week_key: str = None) -> dict:
    ...
    try:
        ...
        users_with_pending = (
            db.session.query(NewsletterDigestQueue.user_id)
            .filter(NewsletterDigestQueue.week_key == week_key, not NewsletterDigestQueue.sent)
            .distinct()
            .all()
        )
...
def _send_single_digest(user_id: int, week_key: str) -> dict:
    ...
    try:
        ...
        queue_entries = NewsletterDigestQueue.query.filter_by(user_id=user_id, week_key=week_key, sent=False).all()
        ...
        active_sub = UserSubscription.query.filter_by(email=user.email, active=True).first()
        ...
        try:
            response = requests.post(webhook_url, headers=request_headers, json=payload, timeout=20)
...
def check_writer_has_publishable_content(newsletter_id: int, journalist_id: int) -> bool:
    ...
    """
    commentary = NewsletterCommentary.query.filter_by(newsletter_id=newsletter_id, author_id=journalist_id).first()
    ...
    try:
        ...
        for commentary in commentaries:
            if commentary.author and commentary.author.is_journalist:
                has_content = check_writer_has_publishable_content(newsletter.id, commentary.author.id)
...
def check_writer_submission_status(journalist_id: int, week_start_date=None) -> dict:
    ...
    try:
        ...
        assigned_teams = JournalistTeamAssignment.query.filter_by(user_id=journalist_id).all()

        team_ids = [assignment.team_id for assignment in assigned_teams]

        newsletters = Newsletter.query.filter(
            Newsletter.team_id.in_(team_ids), Newsletter.week_start_date == target_date
        ).all()

        submission_status = []

        for newsletter in newsletters:
            has_content = check_writer_has_publishable_content(newsletter.id, journalist_id)
