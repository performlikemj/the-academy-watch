...
class FeederService:
    ...
    def get_competition_teams(self, league_api_id: int, season: int) -> list[dict[str, Any]]:
        ...
        if teams:
            profiles = {
                tp.team_id: tp
                for tp in TeamProfile.query.filter(TeamProfile.team_id.in_([t["team_api_id"] for t in teams])).all()
            }
            for t in teams:
    ...
    def get_squad_origins(
        ...
        journeys_by_player = {}
        existing_journeys = PlayerJourney.query.filter(PlayerJourney.player_api_id.in_(player_api_ids)).all()
        ...
        for player in squad_players:
            ...
            resolved = self._resolve_parent_id(strip_youth_suffix(journey.current_club_name or ""))
        ...
        national_keys = [
            k for k, players in academy_groups.items() if is_national_team(players[0]["academy_club_name"])
        ]
        ...
        for academy_id in list(academy_groups.keys()):
            if academy_id not in academy_groups:
                continue
            display_name = strip_youth_suffix(academy_groups[academy_id][0]["academy_club_name"])
    ...
    def _fetch_squad(self, team_api_id: int, season: int, league_api_id: int | None = None) -> list[dict[str, Any]]:
    ...
    def _resolve_parent_id(self, base_name: str) -> int | None:
        ...
        profile = (
            ...
            .order_by(db.func.length(TeamProfile.name).desc())
            .first()
        )
        if profile:
            return profile.team_id
    ...
    def _get_team_logo(self, team_api_id: int) -> str | None:
