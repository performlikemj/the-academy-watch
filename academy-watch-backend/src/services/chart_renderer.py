...
def generate_chart_id(block: dict, player_id: int | None, week_start: str | None, week_end: str | None) -> str:
...
def render_radar_chart(data: dict[str, Any], width: int = 480, height: int = 480) -> bytes:
    ...
    if is_league_format:
        ...
        if has_overlay:
            ...
            legend_elements = [
                ...
                Line2D([0], [0], color=avg_color, linewidth=1.5, linestyle="--", label=f"{league_name} Avg"),
            ]
            ax.legend(
                handles=legend_elements, loc="lower right", fontsize=7, bbox_to_anchor=(1.25, -0.1), framealpha=0.8
            )
        else:
            ...
            ax.legend(
                handles=[Line2D([0], [0], color=color, linewidth=2, label="Player only (no league overlay)")],
                loc="lower right",
                fontsize=7,
    ...
    if is_league_format:
        if has_overlay and league_name:
            footer = (
                f"Per 90 vs {league_peers} {position_group_label.lower()}s in {league_name}. 100% = best in league."
            )
        elif league_name:
            footer = (
            ...
            footer = "League comparison unavailable. Showing player-only per 90 stats normalised to peak axis."
...
def render_line_chart(data: dict[str, Any], width: int = 560, height: int = 320) -> bytes:
    ...
    # placeholder instead.
    has_any_value = any(d.get(stat) is not None and d.get(stat) > 0 for d in line_data for stat in stat_keys)
    if not has_any_value:
        return _render_empty_chart(
            f"No {', '.join(stat_keys)} data available",
            width,
            height,
        )
...
def render_match_cards_summary(data: dict[str, Any], width: int = 760, height: int = 220) -> bytes:
    ...
    wins = sum(
        1
        for f in fixtures
        if (f.get("is_home") and f["home_team"]["score"] > f["away_team"]["score"])
        or (not f.get("is_home") and f["away_team"]["score"] > f["home_team"]["score"])
    )
    ...
    ax.text(
        0.5,
        0.92,
        f"{player_name} - {len(fixtures)} Match Summary",
        fontsize=12,
        fontweight="bold",
        ha="center",
...
def _render_empty_chart(message: str, width: int, height: int) -> bytes:
    ...
    ax.axis("off")
    ax.text(
        0.5, 0.5, message, fontsize=12, ha="center", va="center", color=CHART_COLORS["gray"], transform=ax.transAxes
    )
...
def save_chart_to_file(
    chart_type: str, data: dict[str, Any], filename: str, width: int = 500, height: int = 300
) -> str:
    """
