...
def upgrade():
    if not table_exists('player_links'):
        op.create_table(
            'player_links',
    ...
    create_index_safe('ix_player_links_player_id', 'player_links', ['player_id'])
    create_index_safe('ix_player_links_status', 'player_links', ['status'])
