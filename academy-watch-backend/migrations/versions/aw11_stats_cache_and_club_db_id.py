...
def upgrade():
    ...
    op.create_table(
        ...
        sa.UniqueConstraint('player_api_id', 'team_api_id', 'season',
                            name='uq_player_stats_cache'),
    )
    op.create_index('ix_player_stats_cache_player_api_id',
                    'player_stats_cache', ['player_api_id'])

    # Add current_club_db_id FK to tracked_players
    op.add_column('tracked_players',
                  sa.Column('current_club_db_id', sa.Integer(), nullable=True))
    op.create_foreign_key(
        'fk_tracked_players_current_club',
        'tracked_players', 'teams',
        ['current_club_db_id'], ['id'],
    )


def downgrade():
    op.drop_constraint('fk_tracked_players_current_club',
                       'tracked_players', type_='foreignkey')
    op.drop_column('tracked_players', 'current_club_db_id')
    op.drop_index('ix_player_stats_cache_player_api_id',
                  table_name='player_stats_cache')
