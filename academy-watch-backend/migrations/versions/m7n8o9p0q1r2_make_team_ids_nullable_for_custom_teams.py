...
def upgrade():
    ...
    with op.batch_alter_table("loaned_players", schema=None) as batch_op:
        ...
        batch_op.alter_column("primary_team_id", existing_type=sa.INTEGER(), nullable=True)
        batch_op.alter_column("loan_team_id", existing_type=sa.INTEGER(), nullable=True)

        # Recreate foreign key constraints (now nullable)
        batch_op.create_foreign_key("loaned_players_primary_team_id_fkey", "teams", ["primary_team_id"], ["id"])
        batch_op.create_foreign_key("loaned_players_loan_team_id_fkey", "teams", ["loan_team_id"], ["id"])
...
def downgrade():
    ...
    with op.batch_alter_table("loaned_players", schema=None) as batch_op:
        ...
        batch_op.alter_column("primary_team_id", existing_type=sa.INTEGER(), nullable=False)
        batch_op.alter_column("loan_team_id", existing_type=sa.INTEGER(), nullable=False)
