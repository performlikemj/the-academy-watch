...
def upgrade():
    with op.batch_alter_table("user_accounts", schema=None) as batch_op:
        batch_op.alter_column(
            "profile_image_url", existing_type=sa.VARCHAR(length=255), type_=sa.Text(), existing_nullable=True
        )
...
def downgrade():
    with op.batch_alter_table("user_accounts", schema=None) as batch_op:
        batch_op.alter_column(
            "profile_image_url", existing_type=sa.Text(), type_=sa.VARCHAR(length=255), existing_nullable=True
        )
