...
def upgrade():
    if not table_exists("academy_cohorts"):
        op.create_table(
            "academy_cohorts",
            ...
            sa.UniqueConstraint("team_api_id", "league_api_id", "season", name="uq_academy_cohort"),
        )

    create_index_safe("ix_academy_cohorts_team", "academy_cohorts", ["team_api_id"])

    if not table_exists("cohort_members"):
        op.create_table(
            "cohort_members",
            sa.Column("id", sa.Integer(), nullable=False),
    ...
    create_index_safe("ix_cohort_members_cohort", "cohort_members", ["cohort_id"])
    create_index_safe("ix_cohort_members_player", "cohort_members", ["player_api_id"])
