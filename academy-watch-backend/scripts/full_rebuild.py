...
def stage_4_tracked_players(team_ids, seasons, dry_run):
    ...
    for api_team_id in team_ids:
        ...
        if cohort_ids:
            ...
            print("    Source 3 (cohorts): no cohorts found")
...
def stage_6_refresh_statuses(team_ids, dry_run):
    ...
    for tp in tracked:
        ...
        changed = (
            tp.status != status
            or tp.current_club_api_id != current_club_api_id
            or tp.current_club_name != current_club_name
        )
...
def main():
    ...
    parser.add_argument("--teams", type=str, default=None, help="Comma-separated API team IDs (default: all Big 6)")
    parser.add_argument(
        "--seasons", type=str, default=None, help="Comma-separated seasons (default: 2020,2021,2022,2023,2024)"
    )
    parser.add_argument(
        "--league", type=int, default=None, help="API league ID to auto-discover teams (e.g. 39 for Premier League)"
    )
    parser.add_argument(
        "--exclude-teams", type=str, default=None, help="Comma-separated API team IDs to exclude (use with --league)"
    )
    parser.add_argument("--skip-clean", action="store_true", help="Skip stage 1 (keep existing data)")
    parser.add_argument("--skip-cohorts", action="store_true", help="Skip stage 3 (cohort discovery)")
