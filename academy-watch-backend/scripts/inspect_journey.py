...
with app.app_context():
    ...
    for i, stop in enumerate(data["stops"]):
        print(
            f"  Stop {i}: {stop['club_name']:<30} years={stop['years']:<12} levels={stop['levels']}  apps={stop['total_apps']}"
        )
    ...
    for i in range(1, len(nodes)):
        if nodes[i]["season"] < nodes[i - 1]["season"]:
            found = True
            print(
                f"  Node {nodes[i - 1]['id']} ({nodes[i - 1]['season']} {nodes[i - 1]['club']}) -> Node {nodes[i]['id']} ({nodes[i]['season']} {nodes[i]['club']}) -- GOES BACKWARD"
            )
    if not found:
