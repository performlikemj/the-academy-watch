...
from src.models.league import EmailToken, League, Team, UserSubscription, db
...
class TestGetSubscriptions:
    ...
    def test_get_subscriptions_returns_list(self, subscriptions_app, subscriptions_client, sample_subscription):
        ...
        res = subscriptions_client.get("/api/subscriptions", headers=_admin_headers(subscriptions_app))
    ...
    def test_get_subscriptions_filter_active_only(self, subscriptions_app, subscriptions_client, sample_subscription):
        ...
        res = subscriptions_client.get("/api/subscriptions?active_only=true", headers=_admin_headers(subscriptions_app))
        assert res.status_code == 200
        data = res.get_json()
...
class TestCreateSubscription:
    ...
    def test_create_subscription_success(self, subscriptions_client, sample_team):
        ...
        res = subscriptions_client.post("/api/subscriptions", json={"email": "new@example.com", "team_id": sample_team})
    ...
    def test_create_subscription_missing_fields(self, subscriptions_client):
        ...
        res = subscriptions_client.post("/api/subscriptions", json={"email": "new@example.com"})
...
class TestBulkCreateSubscriptions:
    ...
    def test_bulk_create_success(self, subscriptions_client, sample_team):
        ...
        res = subscriptions_client.post(
            "/api/subscriptions/bulk_create", json={"email": "bulk@example.com", "team_ids": [sample_team]}
        )
        assert res.status_code == 201
        data = res.get_json()
        assert data["created_count"] >= 1
    ...
    def test_bulk_create_missing_fields(self, subscriptions_client):
        """Should return 400 for missing required fields."""
        res = subscriptions_client.post("/api/subscriptions/bulk_create", json={"email": "bulk@example.com"})
        assert res.status_code == 400
...
class TestUnsubscribeByEmail:
    ...
    def test_unsubscribe_success(self, subscriptions_client, sample_subscription):
        ...
        res = subscriptions_client.post("/api/subscriptions/unsubscribe", json={"email": "test@example.com"})
        assert res.status_code == 200
        data = res.get_json()
        assert data["count"] >= 1
    ...
    def test_unsubscribe_no_match(self, subscriptions_client):
        ...
        res = subscriptions_client.post("/api/subscriptions/unsubscribe", json={"email": "nonexistent@example.com"})
        assert res.status_code == 200
        data = res.get_json()
        assert data["count"] == 0
    ...
    def test_unsubscribe_missing_email(self, subscriptions_client):
        ...
        res = subscriptions_client.post("/api/subscriptions/unsubscribe", json={})
        assert res.status_code == 400
...
class TestTokenUnsubscribe:
    ...
    def test_token_unsubscribe_post_success(self, subscriptions_client, sample_subscription):
        ...
        res = subscriptions_client.post(f"/api/subscriptions/unsubscribe/{sample_subscription['token']}")
        assert res.status_code == 200
        data = res.get_json()
    ...
    def test_token_unsubscribe_post_invalid_token(self, subscriptions_client):
        ...
        res = subscriptions_client.post("/api/subscriptions/unsubscribe/invalid-token-123")
    ...
    def test_token_unsubscribe_get_renders_page(self, subscriptions_client, sample_subscription):
        ...
        res = subscriptions_client.get(f"/api/subscriptions/unsubscribe/{sample_subscription['token']}")
        assert res.status_code == 200
        # Should return HTML
...
class TestOneClickUnsubscribe:
    ...
    def test_one_click_unsubscribe_invalid_token_still_returns_200(self, subscriptions_client):
        ...
        res = subscriptions_client.post(
            "/api/subscriptions/one-click-unsubscribe/invalid-token-xyz", data="List-Unsubscribe=One-Click"
        )
...
class TestDeleteSubscription:
    ...
    def test_delete_subscription_success(self, subscriptions_client, sample_subscription):
        ...
        res = subscriptions_client.delete(f"/api/subscriptions/{sample_subscription['id']}")
        assert res.status_code == 200
        data = res.get_json()
        assert data["message"] == "Unsubscribed successfully"
...
class TestManageSubscriptions:
    ...
    def test_request_manage_link_success(self, subscriptions_client):
        ...
        res = subscriptions_client.post("/api/subscriptions/request-manage-link", json={"email": "manage@example.com"})
        assert res.status_code == 200
        data = res.get_json()
    ...
    def test_request_manage_link_missing_email(self, subscriptions_client):
        """Should return 400 for missing email."""
        res = subscriptions_client.post("/api/subscriptions/request-manage-link", json={})
        assert res.status_code == 400
...
class TestMySubscriptions:
    ...
    def test_my_subscriptions_returns_list(self, subscriptions_app, subscriptions_client, sample_team):
        ...
        res = subscriptions_client.get(
            "/api/subscriptions/me", headers=_user_headers(subscriptions_app, "user@example.com")
        )
        assert res.status_code == 200
        data = res.get_json()
        assert isinstance(data, list)
    ...
    def test_update_my_subscriptions_requires_auth(self, subscriptions_client):
        """Should require authentication."""
        res = subscriptions_client.post("/api/subscriptions/me", json={"team_ids": [1]})
        assert res.status_code == 401
