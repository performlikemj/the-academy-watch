...
from src.models.league import League, LoanedPlayer, LoanFlag, Team, db
...
class TestTerminateLoan:
    ...
    def test_terminate_loan_not_found(self, loans_app, loans_client):
        ...
        res = loans_client.post(
            "/api/loans/99999/terminate", json={"reason": "Test"}, headers=_admin_headers(loans_app)
        )
...
class TestUpdateLoanPerformance:
    ...
    def test_update_performance_not_found(self, loans_app, loans_client):
        """Should return 404 for non-existent loan."""
        res = loans_client.put("/api/loans/99999/performance", json={"goals": 1}, headers=_admin_headers(loans_app))
        assert res.status_code == 404
...
class TestLoanFlags:
    ...
    def test_create_flag_missing_fields(self, loans_client):
        ...
        res = loans_client.post("/api/loans/flags", json={"player_id": 123})
    ...
    def test_list_pending_flags_success(self, loans_app, loans_client):
        ...
        res = loans_client.get("/api/loans/flags/pending", headers=_admin_headers(loans_app))
        assert res.status_code == 200
        data = res.get_json()
        assert isinstance(data, list)
        assert len(data) >= 1
    ...
    def test_resolve_flag_not_found(self, loans_app, loans_client):
        ...
        res = loans_client.post(
            "/api/loans/flags/99999/resolve", json={"action": "resolved"}, headers=_admin_headers(loans_app)
        )
        assert res.status_code == 404
...
class TestWeeklyReport:
    ...
    def test_weekly_report_requires_params(self, loans_app, loans_client):
        ...
        res = loans_client.get("/api/loans/weekly-report", headers=_admin_headers(loans_app))
        assert res.status_code == 400
        data = res.get_json()
        assert "error" in data
