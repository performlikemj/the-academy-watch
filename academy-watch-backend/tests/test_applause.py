...
from src.models.league import CommentaryApplause, Newsletter, NewsletterCommentary, Team, UserAccount, db
...
def test_applaud_flow(client, app):
    ...
    with app.app_context():
        ...
        # Create journalist
        journo = UserAccount(
            email="journo@example.com", display_name="Journalist", display_name_lower="journalist", is_journalist=True
        )
        db.session.add(journo)
        db.session.commit()

        # Create Newsletter
        news = Newsletter(team_id=team.id, title="Man Utd Weekly", content="Content", public_slug="slug")
        db.session.add(news)
        db.session.commit()
