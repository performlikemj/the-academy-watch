...
def test_wiki_loan_diff_auto_discovers_titles(app, client, monkeypatch):
    ...
    monkeypatch.setattr(
        "src.admin.sandbox_tasks.search_wikipedia_title",
        lambda query, context="": "Sample Player" if "Sample Player" in query else "Manchester United F.C.",
    )
    monkeypatch.setattr("src.admin.sandbox_tasks.fetch_wikitext", lambda title: "stub")
    monkeypatch.setattr(
        "src.admin.sandbox_tasks.extract_team_loan_candidates",
