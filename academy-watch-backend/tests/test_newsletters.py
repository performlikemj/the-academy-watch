...
def test_lint_and_enrich_infers_player_id_from_latest_lookup(app, monkeypatch):
    with app.app_context():
        ...
        monkeypatch.setattr(
            weekly_agent,
            "_team_logo_for_player",
            lambda pid, loan_team_name=None: f"https://cdn.example.com/teams/{pid}.png",
        )
...
def test_compose_weekly_builds_per_player_reports_with_history(app, monkeypatch):
    ...
    with app.app_context():
        ...
        monkeypatch.setattr(
            weekly_nl_agent,
            "_summarize_team_with_groq",
            lambda team_name, window, players: f"{team_name} overview via Groq.",
        )
        monkeypatch.setattr(
            weekly_nl_agent,
            "api_client",
...
def test_newsletter_web_render_includes_social_meta(app, client, monkeypatch):
    ...
    assert (
        '<meta name="description" content="Goals, minutes, and form tracker for every United loanee this week.">'
        in html
    )
