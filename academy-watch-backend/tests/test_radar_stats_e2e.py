...
def _find_test_players(min_fixtures=5):
    ...
    rows = (
        FixturePlayerStats.query.with_entities(
...
class TestRadarStatsE2E:
    ...
    def test_per90_math_verification(self, app_ctx, test_players):
        ...
        for group, (pid, fp, count) in list(test_players.items())[:3]:
            # Get raw fixtures
            stats_rows = (
                FixturePlayerStats.query.filter_by(player_api_id=pid).filter(FixturePlayerStats.minutes > 0).all()
            )
            ...
            assert per90.get("goals", 0) == expected_goals_p90, (
                f"Goals per-90 mismatch for player {pid}: expected {expected_goals_p90}, got {per90.get('goals', 0)}"
            )
            assert per90.get("assists", 0) == expected_assists_p90, f"Assists per-90 mismatch for player {pid}"
