...
from src.models.league import JournalistSubscription, Newsletter, NewsletterCommentary, Team, UserAccount, db
...
def test_journalist_flow(client, app):
    with app.app_context():
        ...
        subscriber = UserAccount(
            email="subscriber@example.com", display_name="Subscriber Sam", display_name_lower="subscriber sam"
        )
    ...
    with app.app_context():
        sub = JournalistSubscription(subscriber_user_id=subscriber_id, journalist_user_id=journalist_id)
        db.session.add(sub)
        db.session.commit()

        # Verify subscription
        assert (
            JournalistSubscription.query.filter_by(
                subscriber_user_id=subscriber_id, journalist_user_id=journalist_id, is_active=True
            ).first()
            is not None
        )
    ...
    with app.app_context():
        ...
        newsletter = Newsletter(
            ...
            week_end_date=datetime.now(UTC),
        ...
        assert n_dict["commentaries"][0]["is_premium"]
