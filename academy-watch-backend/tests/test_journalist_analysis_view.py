...
def _make_user(email="test@example.com"):
    u = UserAccount(email=email, display_name="Test User", display_name_lower="test user")
    db.session.add(u)
    db.session.commit()
    return u
...
def test_premium_masking_unsubscribed(app, client):
    ...
    commentaries = data.get("commentaries", [])
    assert len(commentaries) == 1
    assert commentaries[0]["is_locked"]
    ...
    resp = client.get(f"/api/newsletters/{newsletter.id}?journalist_id=1", headers={"Authorization": f"Bearer {token}"})
    assert resp.status_code == 200
    data = resp.get_json()
    commentaries = data.get("commentaries", [])
    assert commentaries[0]["is_locked"]
...
def test_premium_access_subscribed(app, client):
    ...
    commentaries = data.get("commentaries", [])
    assert len(commentaries) == 1
    assert not commentaries[0]["is_locked"]
