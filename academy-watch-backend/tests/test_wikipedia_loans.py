...
def test_extract_wikipedia_players_for_season_filters_current_year():
    loans = extract_wikipedia_players(SENIOR_CAREER_TABLE, season_year=2025)
...
def test_extract_wikipedia_players_deduplicates_and_maps_parent():
    loans = extract_wikipedia_players(SENIOR_CAREER_TABLE, season_year=2025)
    assert loans
    for row in loans:
...
def test_collect_player_loans_from_wikipedia(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.utils.wikipedia_players.search_wikipedia_title",
        fake_search,
    )
    monkeypatch.setattr(
        "src.utils.wikipedia_players.fetch_wikitext",
        fake_fetch,
    )
