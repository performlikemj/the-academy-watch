...
usage() {
  echo "Usage: run.sh [--journey <file>]... [--destination <dest>] [--out <dir>] [--grade|--no-grade]"
...
scheme=${SIM_SCHEME:-AcademyWatchExperience}
container_args=()
...
ui_test_target=${SIM_UI_TEST_TARGET:-AcademyWatchUITests}
...
for (const file of files) {
  ...
  let journey
  try { journey = JSON.parse(bytes) } catch { throw new Error(`journey JSON is invalid: ${file}`) }
  if (!['1.1', '1.2', '1.3'].includes(String(journey.version))) throw new Error(`journey must use version 1.1, 1.2 or 1.3: ${file}`)
  ...
  for (const step of journey.steps) {
    ...
    if (!['launch','tap','type','clearAndType','swipe','waitFor','assertVisible','screenshot','dismissKeyboard','systemAlert', ...(String(journey.version) === '1.3' ? ['assertAbsent','assertSelected'] : [])].includes(action)) {
    ...
    if (['tap','waitFor','assertVisible','assertAbsent'].includes(action)) {
    ...
    if (['assertAbsent','assertSelected'].includes(action)) {
      if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error(`${action} requires an object in ${file}`)
      const allowed = action === 'assertSelected' ? ['id','timeout'] : ['id','text','why','timeout']
      if (Object.keys(value).some((key) => !allowed.includes(key))) throw new Error(`unknown ${action} field in ${file}`)
      if (action === 'assertSelected' && (typeof value.id !== 'string' || !value.id.length)) throw new Error(`assertSelected requires id in ${file}`)
      if (action === 'assertAbsent' && Object.hasOwn(value, 'id') && Object.hasOwn(value, 'text')) throw new Error(`assertAbsent requires exactly one locator in ${file}`)
      if (value.timeout !== undefined && (typeof value.timeout !== 'number' || !Number.isFinite(value.timeout) || value.timeout <= 0)) throw new Error(`${action} timeout must be positive in ${file}`)
    }
  ...
  plan.push({ name: journey.name, version: String(journey.version), checkpoint_step_id: journey.steps.find((step) => step.checkpoint === true).id, hash, receipt_path: receiptPath, checkpoint_only: checkpointOnly, preconditions })
}
...
if [ -f "$APP_ROOT/sim/stories.json" ]; then
  if [ -z "$harness_root" ]; then echo "run.sh: HARNESS_ROOT is required for story coverage" >&2; exit 2; fi
  node "$harness_root/checks/story-map.mjs" check "$APP_ROOT" || exit 2
fi
...
echo "iOS sim: app=$app_name scheme=$scheme target=$ui_test_target journeys=${#resolved_journeys[@]}"
if ! (
  cd "$APP_ROOT" || exit 2
...
run_test() {
  (
    ...
    xcodebuild -xctestrun "$run_configuration" -destination "$destination" \
      ...
      test-without-building -only-testing:"$ui_test_target/JourneyRunnerUITests/testJourneys"
  ) > "$test_log" 2>&1
}
if ! run_test; then
...
for (const entry of plan) {
  ...
  if (result.run_mode !== undefined && result.run_mode !== (entry.checkpoint_only ? 'checkpoint' : 'full')) throw new Error(`journey run_mode mismatch: ${entry.name}`)
  expectedCount += 1
  ...
  if (entry.checkpoint_only) {
    if (!result.checkpoint_reached && !(entry.version === '1.3' && result.steps.some((step) => step.ok === false))) throw new Error(`checkpoint receipt was not earned: ${entry.name}`)
    const checkpoint = result.steps.find((step) => step.id === result.steps.at(-1)?.id)
    receipt = {
      journey_hash: entry.hash,
      status: result.checkpoint_reached ? 'created' : 'failed',
      checkpoint_step_id: entry.checkpoint_step_id || checkpoint.id,
      fixture_evidence: result.fixture_evidence,
      recorded_at: runAt,
    }
    if (result.checkpoint_reached) fs.writeFileSync(entry.receipt_path, `${JSON.stringify(receipt, null, 2)}\n`)
  } else {
  ...
  journeyRecords.push({
    ...
    ...(result.run_mode === undefined ? {} : { run_mode: result.run_mode }),
    fixture_evidence: result.fixture_evidence,
...
fs.writeFileSync(path.join(reportDir, 'steps.json'), `${JSON.stringify({ version: 2, selected_journeys: plan.map(({name,hash}) => ({name,hash})), attachment_count: expectedCount, attachment_bytes: expectedBytes, journeys: journeyRecords }, null, 2)}\n`)
...
node "$SIM_DIR/stories-proof.mjs" --report-dir "$staging" --app-dir "$APP_ROOT" || exit 2
...
checker_args=(--strict --app-dir "$APP_ROOT")
