...
final class CompareViewModelTests: XCTestCase {
    ...
    @MainActor
    func testReloadsComparisonWhenAvailabilityIsDisabled() async throws {
        let response: CompareResponse = try decodeFixture(named: "scout_compare_gk_outfielder")
        let client = StubCompareAPIClient(response: response)
        let viewModel = CompareViewModel(playerIDs: [145_060, 403_064], apiClient: client)

        await viewModel.setIncludeAvailability(false)

        XCTAssertFalse(viewModel.includeAvailability)
        let request = await client.capturedRequest()
        XCTAssertEqual(request?.includeAvailability, false)
    }
