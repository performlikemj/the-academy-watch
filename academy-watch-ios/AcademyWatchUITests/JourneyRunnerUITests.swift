...
final class JourneyRunnerUITests: XCTestCase {
    ...
    private struct Action: Decodable {
        ...
        let assertAbsent: PredicateAction?
        let assertSelected: PredicateAction?
        ...
        var populatedCount: Int {
            [launch != nil, tap != nil, type != nil, clearAndType != nil, swipe != nil,
             waitFor != nil, assertVisible != nil, assertAbsent != nil, assertSelected != nil, screenshot != nil,
    ...
    private struct PredicateAction: Decodable {
        let id: String?
        let text: String?
        let why: String?
        let timeout: TimeInterval?

        var locator: LocatorAction { LocatorAction(id: id, text: text, why: why) }
    }
    ...
    private func run(_ loaded: LoadedJourney) throws {
        ...
        guard ["1.1", "1.2", "1.3"].contains(journey.version) else {
            throw RunnerError.invalidJourney("Unsupported journey version \(journey.version); expected 1.1, 1.2 or 1.3.")
        }
        ...
        for (index, step) in journey.steps.enumerated() {
            ...
            do {
                ...
                let isPredicate = step.action.assertAbsent != nil || step.action.assertSelected != nil
                if isPredicate {
                    guard journey.version == "1.3", app.state == .runningForeground else {
                        throw RunnerError.invalidJourney("v1.3 predicate requires a foreground app.")
                    }
                    if step.action.assertAbsent != nil && !previousSettled {
                        throw RunnerError.missingElement("Absence requires the previous action to settle successfully.")
                    }
                }
                ...
                if isPredicate {
                    guard settled.reason != "timeout", app.state == .runningForeground else {
                        throw RunnerError.missingElement("Predicate capture failed: settle timeout or app not foreground.")
                    }
                    // Capture and immediately recheck the predicate; a quiet frame alone is not proof.
                    settledPNG = XCUIScreen.main.screenshot().pngRepresentation
                    if let assertion = step.action.assertAbsent {
                        let (target, _) = try located(assertion.locator, app: app)
                        guard !target.exists else {
                            throw RunnerError.missingElement("Absent target reappeared at capture: \(locatorDescription(assertion.locator)).")
                        }
                    }
            ...
            previousSettled = ok && settleReason != "timeout"
        ...
        let result = JourneyResult(
            ...
            runMode: journey.checkpointOnly ? "checkpoint" : "full",
            steps: results,
    ...
    private func perform(_ action: Action, app: XCUIApplication) throws -> String? {
        ...
        if let assertion = action.assertAbsent {
            let timeout = assertion.timeout ?? 3
            guard timeout.isFinite && timeout > 0 else {
                throw RunnerError.invalidJourney("assertAbsent timeout must be positive.")
            }
            let (target, why) = try located(assertion.locator, app: app)
            let deadline = Date().addingTimeInterval(timeout)
            while target.exists && deadline.timeIntervalSinceNow > 0 {
                waitForInterval(min(0.05, max(0, deadline.timeIntervalSinceNow)))
            }
            guard !target.exists else {
                throw RunnerError.missingElement("Absent target \(locatorDescription(assertion.locator)) still exists after \(timeout) seconds.")
            }
            return why
        }
        if let assertion = action.assertSelected {
            let timeout = assertion.timeout ?? 3
            guard let id = assertion.id, !id.isEmpty, assertion.text == nil, assertion.why == nil,
        ...
        if action.dismissKeyboard != nil {
            ...
            for label in ["Search", "search", "Done", "Return"] {
    ...
    private func located(_ locator: LocatorAction, app: XCUIApplication) throws -> (XCUIElement, String?) {
        ...
        if let id = locator.id { return (element(withID: id, app: app), nil) }
    ...
    private func settleElement(_ locator: LocatorAction, app: XCUIApplication) throws -> XCUIElement {
        ...
        if let id = locator.id { return element(withID: id, app: app) }
        let text = locator.text ?? ""
        let predicate = NSPredicate(format: "label == %@ OR value == %@", text, text)
